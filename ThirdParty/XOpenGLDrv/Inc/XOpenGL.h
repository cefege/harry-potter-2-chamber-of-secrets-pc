...
#define XOPENGL_MODIFIED_LOCK 0
...
inline glm::vec4 FPlaneToVec4(const FVector& Vector)
{
...
# endif
	...
	static UBOOL SelectedGLVersion;
	...
	GLuint UICompositeFBO;
	GLuint UICompositeColorTexture;
	GLuint UICompositeDepthAttachment;
	INT    UICompositeWidth;
	INT    UICompositeHeight;
	UBOOL  FrameUsesUIComposite;
	...
	public:		
		...
		void DeleteBuffer()
		{
			...
			if (VaoObjectName)
				glDeleteVertexArrays(1, &VaoObjectName);
			if (BufferObjectName)
	...
	//
	UBOOL CreateOpenGLContext(void* Window, INT NewColorBytes, UBOOL QueryOnly=0);
	void  UnsetRes();
	void  SwapControl();
	...
	void  UpdateUICompositeFBO(INT Width, INT Height);
	void  DestroyUICompositeFBO();
	...
	DWORD GetPolyFlagsAndDrawFlags(DWORD PolyFlags, DWORD& DrawFlags, UBOOL RemoveOccludeIfSolid);
