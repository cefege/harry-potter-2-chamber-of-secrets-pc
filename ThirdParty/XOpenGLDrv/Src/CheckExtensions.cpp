...
void UXOpenGLRenderDevice::CheckExtensions()
{
    ...
    if (GenerateMipMaps)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_EXT_texture_storage found. GenerateMipMaps enabled."));
        }
        else
        {
    ...
    if (UseBindlessTextures)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_IMG_bindless_texture found. UseBindlessTextures enabled."));
        }
        else
        {
    ...
    {
        debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_cull_distance / GL_EXT_clip_cull_distance found."));
    }
    else
    {
    ...
    if (UsePersistentBuffers)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_buffer_storage found. UsePersistentBuffers enabled."));
        }
        else
        {
    ...
    if (UseBufferInvalidation)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_invalidate_subdata found. UseBufferInvalidation enabled."));
        }
        else
        {
    ...
    if (GenerateMipMaps)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_texture_storage found. GenerateMipMaps enabled."));
        }
        else
        {
    ...
    if (UseBindlessTextures)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_bindless_texture found. UseBindlessTextures enabled."));
        }
        else
        {
    ...
    {
        debugf(NAME_Dev, TEXT("XOpenGL: GL_NVX_gpu_memory_info found."));
    ...
    {
        debugf(NAME_Dev, TEXT("XOpenGL: GL_ATI_meminfo found."));
    ...
    if (UseShaderDrawParameters)
    {
        ...
        {
            debugf(NAME_Dev, TEXT("XOpenGL: GL_ARB_shader_draw_parameters and GL_ARB_shader_storage_buffer_object found. UseShaderDrawParameters enabled."));
        }
        else
        {
...
# if _WIN32 // not worth the hassle with GLX, let SDL check if it works for now.
    ...
    {
        debugf(NAME_Dev, TEXT("XOpenGL: WGL_EXT_swap_control found."));
    ...
    {
        debugf(NAME_Dev, TEXT("XOpenGL: WGL_EXT_swap_control_tear found."));
...
# else
    ...
    debugf(NAME_Dev, TEXT("XOpenGL: SDL_GL RED_SIZE:%i GREEN_SIZE:%i BLUE_SIZE:%i ALPHA_SIZE:%i"),r,g,b,a);

    SDL_GL_GetAttribute(SDL_GL_DEPTH_SIZE, &db);
    debugf(NAME_Dev, TEXT("XOpenGL: SDL_GL_DEPTH_SIZE DesiredDepthBits: %i, provided: %i"),DesiredDepthBits, db);

    if (UseSRGBTextures)
    {
        SDL_GL_GetAttribute(SDL_GL_FRAMEBUFFER_SRGB_CAPABLE, &srgb);
        debugf(NAME_Dev, TEXT("XOpenGL: SDL_GL_FRAMEBUFFER_SRGB_CAPABLE: %i"),srgb);
    }
# endif

    INT MaxTextureImageUnits = 0;
    glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &MaxTextureImageUnits);
    debugf(NAME_Dev, TEXT("XOpenGL: MaxTextureImageUnits: %i"), MaxTextureImageUnits);

    INT MaxVertexTextureImageUnits = 0;
    glGetIntegerv(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS, &MaxVertexTextureImageUnits);
    debugf(NAME_Dev, TEXT("XOpenGL: MaxVertexTextureImageUnits: %i"), MaxVertexTextureImageUnits);
	...
	debugf(NAME_Dev, TEXT("XOpenGL: MaxCombinedTextureImageUnits: %i"), MaxCombinedTextureImageUnits);

	INT MaxElementsVertices = 0;
	glGetIntegerv(GL_MAX_ELEMENTS_VERTICES, &MaxElementsVertices);
	debugf(NAME_Dev, TEXT("XOpenGL: MaxElementsVertices: %i"), MaxElementsVertices);

	INT MaxUniformBufferBindings = 0;
	glGetIntegerv(GL_MAX_UNIFORM_BUFFER_BINDINGS, &MaxUniformBufferBindings);
	debugf(NAME_Dev, TEXT("XOpenGL: MaxUniformBufferBindings: %i"), MaxUniformBufferBindings);

	if (OpenGLVersion == GL_Core)
	{
		INT MaxDualSourceDrawBuffers;
		glGetIntegerv(GL_MAX_DUAL_SOURCE_DRAW_BUFFERS, &MaxDualSourceDrawBuffers);
		debugf(NAME_Dev, TEXT("XOpenGL: MaxDualSourceDrawBuffers: %i"), MaxDualSourceDrawBuffers);
	}

	glGetIntegerv(GL_MAX_TEXTURE_SIZE, &MaxTextureSize);
	debugf(NAME_Dev, TEXT("XOpenGL: MaxTextureSize: %i"), MaxTextureSize);
	...
	if (MaxAnisotropy > 0.f)
	{
		...
		debugf(NAME_Dev, TEXT("XOpenGL: MaxAnisotropy = (%f/%f)"), MaxAnisotropy, tmp);
	...
	if (UseAA)
	{
        ...
        if (MaxAASamples < 2)
        {
            debugf(NAME_Warning, TEXT("XOpenGL: multisampling is unavailable (GL_MAX_SAMPLES=%i); disabling UseAA"), MaxAASamples);
            UseAA = false;
            NumAASamples = 0;
        }
        else if (NumAASamples > MaxAASamples)
        {
        ...
        debugf(NAME_Dev, TEXT("XOpenGL: SDL_GL_MULTISAMPLEBUFFERS: %i, requested NumAASamples: %i, provided NumAASamples/MaxSamples: (%i/%i)"), AABuffers, NumAASamples, NumberOfAASamples, MaxAASamples);
#else
        glGetIntegerv(GL_MAX_SAMPLES, &MaxAASamples);
	...
	{
		debugf(NAME_Dev, TEXT("XOpenGL: Enabling UsePrecache for GenerateMipMaps."));
		UsePrecache = 1;
	}
    ...
    debugf(NAME_Dev, TEXT("XOpenGL: GL_NUM_EXTENSIONS found: %i"), NumberOfExtensions);

    // Clipping Planes
    MaxClippingPlanes = 0;
    glGetIntegerv(GL_MAX_CLIP_DISTANCES, &MaxClippingPlanes);
    debugf(NAME_Dev, TEXT("XOpenGL: GL_MAX_CLIP_DISTANCES found: %i"), MaxClippingPlanes);
    ...
    debugf(NAME_Dev, TEXT("XOpenGL: MaxUniformBlockSize: %i"), MaxUniformBlockSize);

    INT MaxOutputComponents = 0;
    glGetIntegerv(GL_MAX_VERTEX_OUTPUT_COMPONENTS, &MaxOutputComponents);
    debugf(NAME_Dev, TEXT("XOpenGL: MaxVertexOutputComponents: %i"), MaxOutputComponents);

    INT MaxVaryingComponents = 0;
    glGetIntegerv(GL_MAX_VARYING_COMPONENTS, &MaxVaryingComponents);
    debugf(NAME_Dev, TEXT("XOpenGL: MaxVaryingComponents: %i"), MaxVaryingComponents);

    INT MaxVaryingVectors = 0;
    glGetIntegerv(GL_MAX_VARYING_VECTORS, &MaxVaryingVectors);
    debugf(NAME_Dev, TEXT("XOpenGL: MaxVaryingVectors: %i"), MaxVaryingVectors);
