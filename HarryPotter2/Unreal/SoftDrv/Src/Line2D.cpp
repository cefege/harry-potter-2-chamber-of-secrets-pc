...
)
{
	...
	if( LineFlags & LINE_DepthCued )
	{
		...
		FCacheItem* Item;
		QWORD CacheID    = MakeCacheID( CID_DepthLineTable, Color.HiColor565(), Viewport->ByteID(), NULL );
		...
		if( ColorTab.PtrVOID == NULL )
		{
			...
			FLOAT GammaCorrection = 0.56f;
			for( int i=0; i<256; i++ )
			{
				FLOAT GFactor = appPow( i/256.0, GammaCorrection );
				if( Viewport->ColorBytes==2 && (Viewport->Caps & CC_RGB565) )
					ColorTab.PtrWORD[i]
					=	( appFloor(256 * C.Z * GFactor / 8.0f) << 0 )
					+	( appFloor(256 * C.Y * GFactor / 4.0f) << 5 )
					+	( appFloor(256 * C.X * GFactor / 8.0f) << 11);
				else if( Viewport->ColorBytes==2 )
					ColorTab.PtrWORD[i]
					=	( appFloor(256 * C.Z * GFactor / 8.0f) << 0 )
					+	( appFloor(256 * C.Y * GFactor / 8.0f) << 5 )
					+	( appFloor(256 * C.X * GFactor / 8.0f) << 10);
				else if( Viewport->ColorBytes==4 )
					ColorTab.PtrDWORD[i]
					=	( appFloor(256 * C.Z * GFactor) << 0 )
					+	( appFloor(256 * C.Y * GFactor) << 8 )
					+	( appFloor(256 * C.X * GFactor) << 16);
				else appErrorf( TEXT("Invalid ColorBytes") );
			}
		}
