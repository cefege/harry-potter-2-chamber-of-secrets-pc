...
#define ALPHAFOG     1		//  Alpha fogging
...
void USoftwareRenderDevice::InitMMXFlashes( FLOAT Brightness, INT ColorBytes, DWORD Caps)
{
	...
	DWORD FR = appRound( Scale * FlashScale.Z );
	DWORD FG = appRound( Scale * FlashScale.Y );
	DWORD FB = appRound( Scale * FlashScale.X );
	...
	MMXFlashOffset13.R = ((DWORD)FlashFog.Z << 4);
	MMXFlashOffset13.G = ((DWORD)FlashFog.Y << 4);
	MMXFlashOffset13.B = ((DWORD)FlashFog.X << 4);
	...
	UnlitMMXValue.R = Clamp( appRound(UnlitLight * FlashScale.X) ,0,0x7FFF);
	UnlitMMXValue.G = Clamp( appRound(UnlitLight * FlashScale.Y) ,0,0x7FFF);
	UnlitMMXValue.B = Clamp( appRound(UnlitLight * FlashScale.Z) ,0,0x7FFF);
	UnlitMMXValue.A = 0;
...
void USoftwareRenderDevice::InitColorTables( FLOAT Brightness, INT ColorBytes, DWORD Caps)
{
	...
	int FogR  = appRound( FlashFog.X * (double)( 1<<20 ) );  // 1<<20  
	int FogG  = appRound( FlashFog.Y * (double)( 1<<20 ) );
	int FogB  = appRound( FlashFog.Z * (double)( 1<<20 ) );

	int D_ScaleR = appRound( Scale * FlashScale.X * (double)(1 << 20));
	int D_ScaleG = appRound( Scale * FlashScale.Y * (double)(1 << 20));
	int D_ScaleB = appRound( Scale * FlashScale.Z * (double)(1 << 20));
...
void USoftwareRenderDevice::DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet, DWORD PolyFlags, BYTE cAlpha  )
//void USoftwareRenderDevice::DrawPolyV( FSceneFrame* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet )
{
	guard(USoftwareRenderDevice::DrawComplexSurface);
	FMemMark Mark(GMem);
	...
	INT IsModulated   = (PolyFlags & PF_Modulated);
	INT IsTranslucent = (PolyFlags & PF_Translucent);
	INT IsMasked      = (PolyFlags & PF_Masked);
	...
	{	
		...
		if (FrameLocksCounter != OldFrameLocks) 
		{
			...
			if
			(	Viewport->GetOuterUClient()->Brightness != SavedBrightness
			...
			||  SavedRenderMode              != RenderMode )
			{
				...
				if (GIsMMX)
				{
					InitMMXFlashes ( Viewport->GetOuterUClient()->Brightness, Viewport->ColorBytes, Viewport->Caps);
				}
				else
				{
					InitColorTables( Viewport->GetOuterUClient()->Brightness, Viewport->ColorBytes, Viewport->Caps);
				}

				SavedFog		= FlashFog;
				SavedScale		= FlashScale;
				SavedColorBytes = Viewport->ColorBytes;
				SavedBrightness = Viewport->GetOuterUClient()->Brightness;
		...
		FMipmapBase* FirstMip = Surface.Texture->Mips[0];
		...
		*/
		...
		if ( (ThisTextureID != LastTextureID) || RefreshSetupFlag ) // Same as last setup ?
		{
			...
			{		
				for( int i=0; i<ARRAY_COUNT(Mips); i++ )
				{
					...
					FMipmapBase& Src	= *Surface.Texture->Mips[iMip];
		...
		{   
			...
			FMipmapBase& SrcMip		= *Surface.LightMap->Mips[0];
		...
		if( Frame->X<=400 ) MipMult *= 0.67f; // initial suggested value 0.6....
		...
		*/
		...
		if ( TextureSmooth && (RenderMode == DRAWNORMAL) && (!(PolyFlags & PF_NoSmooth)))
		{
			...
			if (GIsMMX)
			{
				do // setup multiple lines
				{
					...
					do  // proceed thru spans
					{
						...
						#if DO_GUARD_SLOW
						if (XSize == 0) appErrorf(TEXT("0-size span detected.")); //#debug
						#endif
