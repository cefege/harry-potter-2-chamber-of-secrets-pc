...
guardSlow(FlashTriangle EdgeSetup);
	...
	do {	
		...
		if ( TB->YFloor < TA->YFloor )
		{
			...
			FLOAT DFG     = RDY * (TA->Fog.Y - TB->Fog.Y);
			FLOAT DFB     = RDY * (TA->Fog.Z - TB->Fog.Z);
...
INT USoftwareRenderDevice::MaxVertices()
{
	// Return arbitrarily large number, because we don't buffer.
	return 256;
}

void USoftwareRenderDevice::DrawTriangles( FSceneNode* Frame, FTextureInfo& Texture, FTransTexture** Pts, INT NumPts, _WORD* Indices, INT NumIndices, DWORD PolyFlags, FSpanBuffer* SpanBuffer )
{
	guard(USoftwareRenderDevice::DrawTriangles);

	// Screen Y size not supported if bigger than our setup buffer size:
	if( Viewport->SizeY > MaximumYScreenSize ) 
		return;

	if ( (PolyFlags & (PF_Highlighted|PF_Translucent)) == (PF_Highlighted|PF_Translucent))
	{
		// Render masked textures in preference to translucency or modulation.
	...
	// Allow indexed or non-indexed drawing.
	if( Indices )
		// Indexed tri-list.
		for( INT i=0; i<NumIndices; i+=3 )
		{
			Point0 = Pts[ Indices[i] ];
			Point1 = Pts[ Indices[i+1] ]; 
			Point2 = Pts[ Indices[i+2] ]; 
