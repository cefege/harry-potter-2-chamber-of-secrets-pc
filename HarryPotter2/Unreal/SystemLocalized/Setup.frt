...
MissingProduct=%s nécessite tout d'abord que %s soit installé dans le dossier. Veuillez localiser le fichier existant pour %s, ou quitter et procéder à l'installation maintenant.\n\nVous pouvez cliquer sur le lien Web ci-dessous si vous voulez obtenir des informations sur le produit.
OldVersion=%s nécessite %s la version %i ou une version plus récente, mais la version installée sur votre système est plus ancienne (vous possédez la version %i). Veuillez installer %s version %i ou une version plus récente et réessayer. Vous pouvez cliquer sur le lien Web ci-dessous si vous voulez plus d'informations.\n\nLe programme d'installation va maintenant s'arrêter.
FolderTitle=Dossier "%s"
FolderFormat=Le nom de fichier que vous avez entré, "%s", n'est pas valide. Veuillez réessayer.
...
WrongCd=Le fichier %s voulu %s n'a pas été trouvé dans le lecteur CD-ROM.\n\nVeuillez vérifier que le bon CD se trouve dans le lecteur.
...
PatchCorrupt=Le fichier rustine %s est corrompu.\n\n%s
CdFileMismatch=Le fichier %s se trouvant sur le CD-ROM ne correspond pas à la version attendue. En conséquence, %s ne peut pas profiter de la rustine.

[General]
Space=%i mégaoctets
Ready=Le programme d'installation est prêt à installer %s.\nLe dossier de destination est : %s\n\nCliquez sur "Installer" pour commencer l'installation.\nCliquez sur "Annuler" pour quitter sans procéder à l'installation de %s.
InstallCancel=Annuler installation
CancelPrompt=Attention ! Si vous annulez l'installation maintenant, %s ne fonctionnera pas correctement. Etes-vous sûr(e) de vouloir l'annuler ? 
CdDescription=Pour installer la mise à jour %s, certains fichiers doivent être copiés du CD-ROM original %s.\n\nVeuillez insérer le CD-ROM %s dans le lecteur, et cliquez sur "Suivant". Si la lettre correspondant à votre lecteur CD n'est pas correcte, veuillez la saisir ci-dessous.
CdDrive=Lecteur CD-ROM 
NotEnoughSpaceTitle=Installation impossible à cet endroit
NotEnoughSpace=Il n'y a pas suffisamment d'espace disque disponible sur le lecteur %s pour l'installation de %s. Veuillez sélectionner un lecteur comportant plus d'espace libre, ou quitter, libérer de l'espace, et réessayer.
Finish=&Terminer
...
IDC_WelcomePromptUpdate=Ce programme va mettre à jour votre copie de %s (version %s).\n\nCliquez sur "Suivant" pour lancer le programme d'installation, ou sur "Annuler" pour quitter.\n\nVous trouverez dans la zone ci-dessous des liens Web sur lesquels vous pouvez cliquer si vous souhaitez obtenir plus d'informations.
...
IDC_RestartText=Avant de redémarrer, fermez toutes les applications ouvertes, y compris les sessions MS-DOS, et sauvegardez tous les documents en cours. Retirez la disquette se trouvant à l'intérieur du lecteur, le cas échéant. Cliquez ensuite sur le bouton Redémarrer de Windows.
...
IDC_UninstallPrompt=Ceci va supprimer %s.
IDC_UninstallQuestion=EEtes-vous sûr de vouloir supprimer ces composants ?
