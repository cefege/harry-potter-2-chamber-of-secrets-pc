...
void UEditorEngine::Init()
{
	...
	OCylinder	  = new UOrientedCylinder;
	OvalCylinder  = new UAlignedOvalCylinder;
	OOvalCylinder = new UOrientedOvalCylinder;
	Box			  = new UBox;
...
void UEditorEngine::ResetSound()
{
	...
	//	Ensures all sounds get turned off.  The exec only executes the SetViewport()
...
void UEditorEngine::Tick( float DeltaSeconds )
{
	...
	if( Audio )
	{
		...
		if( AudioViewport )
		{
			AActor*      ViewActor    = AudioViewport->Actor;
			FVector      ViewLocation = ViewActor->Location;
			FRotator     ViewRotation = ViewActor->Rotation;

			FSceneNode* Frame = Render->CreateMasterFrame( AudioViewport, ViewLocation, ViewRotation, NULL );
			Audio->Update( Frame );
...
void SoundTopicHandler::Get( ULevel* Level, const TCHAR* Item, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Item,TEXT("INFO")) )
	{
		TCHAR Name[NAME_SIZE];
	...
	else if ( ParseCommand(&Item,TEXT("SET")) )
	{
		TCHAR Name[NAME_SIZE];
		if( Parse( Item, TEXT("PATHNAME="), Name, ARRAY_COUNT(Name) ) )
		{
			...
			if (pSound && appStrfind (Item, TEXT("MEMBER=")))
			{
				if (appStrfind (Item, TEXT("Streaming")))
				{
					const TCHAR * pValueStr = TEXT("VALUE=");
					INT   ValLength = appStrlen(pValueStr);

					const TCHAR * pStr = appStrfind (Item, pValueStr); 
					if (pStr)
					{
						if (appAtoi(pStr + ValLength))
						{
							pSound->CoreFlags |= SF_Streaming;
						}
						else
						{
							pSound->CoreFlags &= ~SF_Streaming;
...
void SoundTopicHandler::Get( ULevel* Level, const TCHAR* Item, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Item,TEXT("INFO")) )
	{
		...
		if( Parse( Item, TEXT("PATHNAME="), Name, ARRAY_COUNT(Name) ) )
		{
			...
			{
				unsigned short log[64];
				if (appStrfind (Item, TEXT("Looping")))
				{
					appSprintf( log, TEXT("%d"), pSound->Data.bLooping );
					Ar.Log( log );
				}

				if (appStrfind (Item, TEXT("Streaming")))
				{
					appSprintf( log, TEXT("%d"), pSound->bStreamingFromFile );
					Ar.Log( log );
				}

				if (appStrfind (Item, TEXT("Stereo")))
				{
					appSprintf( log, TEXT("%d"), pSound->Data.IsStereo () );
					Ar.Log( log );
				}

				if (appStrfind (Item, TEXT("ULength")))
				{
					if (pSound->bIsMP2)
						appSprintf( log, TEXT("%d"), pSound->Data.UncompressedLength() );
					else
						appSprintf( log, TEXT("%d"), pSound->Data.Length() );
					Ar.Log( log );
				}
		...
		if( Parse( Item, TEXT("PATHNAME="), Name, ARRAY_COUNT(Name) ) )
		{
			...
			if (pSound && appStrfind (Item, TEXT("MEMBER=")))
			{
				if (appStrfind (Item, TEXT("Looping")))
				{
					...
					if (pStr)
					{
						pSound->Data.bLooping = appAtoi(pStr + ValLength);
						if (pSound->Data.bLooping)
							pSound->SetupLoopBuffer ();
					}
				...
				if (appStrfind (Item, TEXT("Streaming")))
				{
					...
					const TCHAR * pStr = appStrfind (Item, pValueStr); 
					if (pStr)
						pSound->bStreamingFromFile = appAtoi(pStr + ValLength);

				}// endif is streaming ?

			}//endif is member ?
		}//endif parse name
	}//endif parsecommand 'Set'
...
void MeshTopicHandler::Get( ULevel* Level, const TCHAR* Item, FOutputDevice& Ar )
{
	...
	if( !appStrnicmp(Item,TEXT("NUMANIMSEQS"),11) )
	{
		...
		if( ParseObject<UMesh>(Item,TEXT("NAME="),Mesh,ANY_PACKAGE) )
		{
			...
			{   
				if( ((USkeletalMesh*)Mesh)->DefaultAnimation )
					AnimNum = ((USkeletalMesh*)Mesh)->DefaultAnimation->AnimSeqs.Num();
			}
			else
				AnimNum = Mesh->AnimSeqs.Num();			
			Ar.Logf( TEXT("%i"), AnimNum );
		}
	}
	else if( !appStrnicmp(Item,TEXT("ANIMSEQ"),7) )
	{
		...
		&&	(Parse(Item,TEXT("NUM="),SeqNum)) )
		{
			...
			{
				if( ((USkeletalMesh*)Mesh)->DefaultAnimation )
					Seq = &((USkeletalMesh*)Mesh)->DefaultAnimation->AnimSeqs(SeqNum);
			}
			else
				Seq = &Mesh->AnimSeqs(SeqNum);
