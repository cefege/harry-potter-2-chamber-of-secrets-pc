...
)
{
	...
	float	PortalBias = ( ( Balance >> 8 ) & 0xFF ) / 100.0f;	// grab half of the first word from Balance (implemented as _WORD in UnEdSrv.cpp) to recover "PortalBias"
	...
	for( i=0; i<NumPolys; i+=Inc )
	{
		...
		Score = ( 100.0f - float(Balance) ) * Splits + float(Balance) * Abs( Front - Back );
		if( Poly->PolyFlags & PF_Portal )
		{
#if 1 //PortalBias -- added by Legend on 4/12/2000
			//
			...
			//
			Score -= ( 100.0f - float(Balance) ) * Splits * PortalBias; // ignore PortalBias of the split polys -- bias toward portal selection for cutting planes!
#else
			Score -= ( 100.0f - float(Balance) ) * Splits; // ignore the split polys -- portals should be selected for cut!
#endif
		}
