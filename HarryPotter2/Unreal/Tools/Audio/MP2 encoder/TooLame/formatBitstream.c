...
static int side_queue_elements (int *forwardFrameLength,
				int *forwardSILength);
static void free_side_queues (void);
static void WriteMainDataBits (unsigned val,
			       unsigned nbits, BF_FrameResults * results);
/*
...
static void
WriteMainDataBits (unsigned val, unsigned nbits, BF_FrameResults * results)
{
  assert (nbits <= 32);
...
store_side_info (BF_FrameData * info)
{
  ...
  for (ch = 0; ch < info->nChannels; ch++)
    {
      l->side_info.channelSIPH[ch] =
	BF_LoadHolderFromBitstreamPart (l->side_info.channelSIPH[ch],
					info->channelSI[ch]);
  ...
  for (gr = 0; gr < info->nGranules; gr++)
    for (ch = 0; ch < info->nChannels; ch++)
      {
	l->side_info.spectrumSIPH[gr][ch] =
	  BF_LoadHolderFromBitstreamPart (l->side_info.spectrumSIPH[gr][ch],
...
*/
BF_PartHolder *
BF_newPartHolder (int max_elements)
{
...
BF_PartHolder *
BF_NewHolderFromBitstreamPart (BF_BitstreamPart * thePart)
{
...
BF_PartHolder *
BF_LoadHolderFromBitstreamPart (BF_PartHolder * theHolder,
...
*/
BF_PartHolder *
BF_resizePartHolder (BF_PartHolder * oldPH, int max_elements)
{
  int elems, i;
  BF_PartHolder *newPH;
...
BF_PartHolder *
BF_freePartHolder (BF_PartHolder * thePH)
{
...
*/
BF_PartHolder *
BF_addElement (BF_PartHolder * thePH, BF_BitstreamElement * theElement)
{
...
*/
BF_PartHolder *
BF_addEntry (BF_PartHolder * thePH, uint32 value, uint16 length)
{
