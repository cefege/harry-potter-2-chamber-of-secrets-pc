...
void psycho_ii_init (double sfreq);

void
psycho_ii (short int *buffer,
	   short int savebuf[1056], int chn, FLOAT snr32[32], double sfreq)
/* to match prototype : float args are always double */
{
  ...
  for (i = 0; i < 2; i++)
      ...
      for (j = 0; j < HBLKSIZE; j++)
	{
       ...
       *****************************************************************************/
#ifdef MFCMISC
      memset (grouped_e, 0, sizeof (FCB));
      memset (grouped_c, 0, sizeof (FCB));
