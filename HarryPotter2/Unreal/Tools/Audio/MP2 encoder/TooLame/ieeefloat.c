      ...
      else
	{
	  if (expon == 0)
	    {			/* Denormalized number */
	      ...
	      f =
		ldexp ((defdouble) mantissa,
		       expon - SEXP_OFFSET - SEXP_POSITION + 1);
	    }
	  else
	    {			/* Normalized number */
	      ...
	      f =
		ldexp ((defdouble) mantissa,
		       expon - SEXP_OFFSET - SEXP_POSITION);
	    }
	}
    }
      ...
      else
	{
	  ...
	  if (expon < -(SEXP_OFFSET - 2))
	    ...
	    {			/* Normalized number */
	      ...
	      bits =
		sign | ((long) ((expon + SEXP_OFFSET - 1)) << SEXP_POSITION) |
		mantissa;
	    }
	}
    }
      ...
      else
	{
	  if (expon == 0)
	    {			/* Denormalized number */
	      ...
	      f =
		ldexp ((defdouble) mantissa,
		       expon - DEXP_OFFSET - DEXP_POSITION + 1);
	      f +=
		ldexp (UnsignedToFloat (second),
		       expon - DEXP_OFFSET - DEXP_POSITION + 1 - 32);
	    }
	  else
	    {			/* Normalized number */
	      ...
	      f =
		ldexp ((defdouble) mantissa,
		       expon - DEXP_OFFSET - DEXP_POSITION);
	      f +=
		ldexp (UnsignedToFloat (second),
		       expon - DEXP_OFFSET - DEXP_POSITION - 32);
	    }
	}
    }
      ...
      else
	{
	  ...
	  if (expon < -(DEXP_OFFSET - 2))
		...
		{		/* Nonzero denormalized number */
		  ...
		  first = sign | mantissa;
		  second =
		    FloatToUnsigned (floor (ldexp (fsMant - mantissa, 32)));
		}
	    }
	    ...
	    {			/* Normalized number */
	      ...
	      first =
		sign | ((long) ((expon + DEXP_OFFSET - 1)) << DEXP_POSITION) |
		mantissa;
	      second =
		FloatToUnsigned (floor (ldexp (fsMant - mantissa, 32)));
	    }
     ...
     defdouble f;
{
  ...
  fprintf (stderr, "float(%g) [%08lX] --> IEEE(%g) [%s]\n",
	   f, p.i, MachineIEEESingle (bytes), hex);
#else /* IEEE */
     ...
     char *hex;
{
  ...
  fprintf (stderr,
	   ...
	   *((defdouble *) (bytes)), hex, hex + 4, hex + 12, f,
     ...
     defdouble f;
{
  ...
  fprintf (stderr, "extended(%g) --> IEEE(%g) [%.4s %.8s %.8s]\n",
	   f, *((defdouble *) (bytes)), hex, hex + 4, hex + 12);
#else /* !Macintosh */
...
SignalFPE (i, j)
     ...
     void (*j) ();
{
  fprintf (stderr, "[Floating Point Interrupt Caught.]\n", i, j);
