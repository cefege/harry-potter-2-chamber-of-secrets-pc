...
extern void maxmnr (double mnr[2][SBLIMIT], char used[2][SBLIMIT],
		    int sblimit, int stereo, int *min_sb, int *min_ch);
int available_bits (layer * info, options * glopts);
#ifdef VBR
int VBR_bits_for_nonoise (double FAR perm_smr[2][SBLIMIT],
			  unsigned int scfsi[2][SBLIMIT],
			  frame_params * fr_ps, int vbrlevel);
extern void VBR_maxmnr (double mnr[2][SBLIMIT], char used[2][SBLIMIT],
			int sblimit, int stereo, int *min_sb, int *min_ch,
			options * glopts);
int VBR_bit_allocation (double FAR perm_smr[2][SBLIMIT],
			unsigned int scfsi[2][SBLIMIT],
			unsigned int bit_alloc[2][SBLIMIT], int *adb,
			frame_params * fr_ps, options * glopts);
#endif
/*=======================================================================\
|                                                                       |
...
void
combine_LR (double FAR sb_sample[2][3][SCALE_BLOCK][SBLIMIT],
...
#ifdef PDS1
void
scale_factor_calc (double FAR sb_sample[][3][SCALE_BLOCK][SBLIMIT],
	...
	for (i = sblimit; i--;)
	  {
	    ...
	    register double cur_max =
	      mod (sb_sample[k][t][SCALE_BLOCK - 1][i]);
...
#else
void
scale_factor_calc (sb_sample, scalar, stereo, sblimit)
...
void
pick_scale (unsigned int scalar[2][3][SBLIMIT],
	    frame_params * fr_ps, double FAR max_sc[2][SBLIMIT])
{
  int i, j, k, max;
	      ...
	      frame_params * fr_ps, Bit_stream_struc * bs)
{
  int stereo = fr_ps->stereo;
  int sblimit = fr_ps->sblimit;
  /* int jsbound = fr_ps->jsbound; not used? MFC Nov99 */
  int i, j, k;
...
int
bits_for_nonoise (double FAR perm_smr[2][SBLIMIT],
		  unsigned int scfsi[2][SBLIMIT], frame_params * fr_ps)
{
  int sb, ch, ba;
  int stereo = fr_ps->stereo;
...
#ifdef VBR
int
VBR_bits_for_nonoise (double FAR perm_smr[2][SBLIMIT],
		      unsigned int scfsi[2][SBLIMIT],
		      frame_params * fr_ps, int vbrlevel)
{
  int sb, ch, ba;
...
#ifdef VBR
void
main_bit_allocation (double FAR perm_smr[2][SBLIMIT],
		     unsigned int scfsi[2][SBLIMIT],
		     unsigned int bit_alloc[2][SBLIMIT],
		     int *adb, frame_params * fr_ps, options * glopts)
#else
void
main_bit_allocation (double FAR perm_smr[2][SBLIMIT],
		     unsigned int scfsi[2][SBLIMIT],
...
#endif
{
  ...
  if (glopts->vbr == FALSE)
    ...
    {
      /* calculate the default allocation */
      fr_ps->header->bitrate_index = 10;
      *adb = available_bits (fr_ps->header, glopts);
      ...
      while (VBR_bits_for_nonoise (perm_smr, scfsi, fr_ps, glopts->vbrlevel) >
	     *adb && fr_ps->header->bitrate_index != 14)
	{
	  //fprintf(stderr, "need:%i avail %i\n",bits_for_nonoise(perm_smr, scfsi, fr_ps),*adb);
	  fr_ps->header->bitrate_index++;
	  if (fr_ps->header->bitrate_index > 14)
	    fr_ps->header->bitrate_index = 14;
	  *adb = available_bits (fr_ps->header, glopts);
	}
      noisy_sbs =
	VBR_bit_allocation (perm_smr, scfsi, bit_alloc, adb, fr_ps, glopts);
    }
#else
  noisy_sbs = a_bit_allocation (perm_smr, scfsi, bit_alloc, adb, fr_ps);
#endif
...
#ifdef VBR
/* make the max search an external function so that I can hack on it cleanly */
extern void
VBR_maxmnr (double mnr[2][SBLIMIT], char used[2][SBLIMIT],
	    int sblimit, int stereo, int *min_sb, int *min_ch,
	    options * glopts)
{
#ifdef HACKIT
...
int
VBR_bit_allocation (double FAR perm_smr[2][SBLIMIT],
		    unsigned int scfsi[2][SBLIMIT],
		    unsigned int bit_alloc[2][SBLIMIT],
		    int *adb, frame_params * fr_ps, options * glopts)
{
  ...
  do
    {
      ...
      VBR_maxmnr (mnr, used, sblimit, stereo, &min_sb, &min_ch, glopts);
...
/* make the max search an external function so that I can hack on it cleanly */
extern void
maxmnr (double mnr[2][SBLIMIT], char used[2][SBLIMIT],
	int sblimit, int stereo, int *min_sb, int *min_ch)
{
  int i, k;
  double small;
...
int
a_bit_allocation (double FAR perm_smr[2][SBLIMIT],
		  unsigned int scfsi[2][SBLIMIT],
		  unsigned int bit_alloc[2][SBLIMIT],
		  int *adb, frame_params * fr_ps)
{
  ...
  *adb -= bbal + berr + banc;
  ...
  do
    {
      /* locate the subband with minimum SMR */
      maxmnr (mnr, used, sblimit, stereo, &min_sb, &min_ch);
...
void
encode_bit_alloc (unsigned int bit_alloc[2][SBLIMIT],
		  frame_params * fr_ps, Bit_stream_struc * bs)
{
  int i, k;
  int stereo = fr_ps->stereo;
...
void
sample_encoding (unsigned int FAR sbband[2][3][SCALE_BLOCK][SBLIMIT],
		 unsigned int bit_alloc[2][SBLIMIT],
		 frame_params * fr_ps, Bit_stream_struc * bs)
{
  unsigned int temp;
