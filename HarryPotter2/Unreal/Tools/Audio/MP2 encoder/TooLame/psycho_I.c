...
void
psycho_i (short FAR buffer[2][1152],
	  double FAR scale[2][SBLIMIT],
	  double FAR ltmin[2][SBLIMIT], frame_params * fr_ps)
{
...
void
read_cbound (lay, freq)		/* this function reads in critical */
...
void
read_freq_band (ltg, lay, freq)	/* this function reads in   */
...
void
make_map (mask FAR power[HAN_SIZE], g_thres FAR * ltg)
...
****************************************************************/
void
hann_fft_pickmax (double sample[FFT_SIZE], mask power[HAN_SIZE],
		  double spike[SBLIMIT], float energy[FFT_SIZE])
{
...
void
tonal_label (power, tone)	/* this function extracts (tonal) */
...
void
noise_label (mask FAR * power, int *noise, g_thres FAR * ltg,
	     float energy[FFT_SIZE])
{
...
#define NOPOW
		  ...
		  if (CF * energy[j] > 0.00001)
		    fprintf (stderr, "[%f %f]  ",
			     pow (10.0, power[j].x / 10.0), CF * energy[j]);
...
void
subsampling (mask FAR power[HAN_SIZE],
	     g_thres FAR * ltg, int *tone, int *noise)
{
  int i, old;
...
void
threshold (mask FAR power[HAN_SIZE],
	   g_thres FAR * ltg, int *tone, int *noise, int bit_rate)
{
  int k, t;
  double dz, tmps, vf;
...
void
minimum_mask (g_thres FAR * ltg, double FAR ltmin[SBLIMIT], int sblimit)
{
  double min;
  int i, j;
...
void
smr (double FAR ltmin[SBLIMIT],
     double FAR spike[SBLIMIT], double FAR scale[SBLIMIT], int sblimit)
{
  int i;
  double max;
