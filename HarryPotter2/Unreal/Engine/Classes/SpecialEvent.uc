...
var() byte       SoundRadius;			// Radius of ambient sound.
var() byte       SoundVolume;			// Volume of amient sound.
var() byte       SoundPitch;			// Sound pitch shift, 64.0=none.
var() bool       UseSoundParameters;	// If true, SoundRadius, SoundVolume, and SoundPitch are used.  Ignored otherwise.
...
state() PlaySoundEffect
{
	...
	{
		Global.Trigger( Self, EventInstigator );
		if (UseSoundParameters)
			PlaySound( Sound, , SoundVolume / 128.0, , SoundRadius * 32, SoundPitch / 64.0 );	// scale sound radius a bit (by 32 ???)
		else
			PlaySound( Sound );
	}
}
...
state() PlayAmbientSoundEffect
{
	function Trigger( actor Other, pawn EventInstigator )
	{
		...
		//	Set the parameters before the sound because the ambient sound is triggered
		//	by the presence of a valid sound in AmbientSound.  The parameters might otherwise
		//	be skipped.
		//
		if (UseSoundParameters)
		{
			EventInstigator.SoundRadius = SoundRadius;
			EventInstigator.SoundVolume = SoundVolume;
			EventInstigator.SoundPitch = SoundPitch;
		}
		EventInstigator.AmbientSound = Sound;
	}
}
