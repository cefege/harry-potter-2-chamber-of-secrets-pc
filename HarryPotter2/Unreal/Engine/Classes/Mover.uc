...
var() enum EMoverEncroachType
{
	...
	ME_ReallyStopWhenEncroach,		// Stop when we hit an actor.
	ME_ReallyReturnWhenEncroach,	// Return to previous position when we hit an actor.
   	ME_ReallyCrushWhenEncroach,		// Crush the poor helpless actor.
	ME_ReallyIgnoreWhenEncroach,	// Ignore encroached actors.
} MoverEncroachType;
...
var() enum EMoverGlideType
{
	...
	MV_SpringByTime,		// Act like a spring in the end
...
//-----------------------------------------------------------------------------
// Movement parameters.
var() float      MoveTimes[8];     // Time to spend moving between keyframes.
...
var() bool       bKeepRotationDirection;   // How much to damage encroached actors.
...
var(MoverSounds) float      MoverRadius;

var(MoverSounds) byte       MoverVolume;
var(MoverSounds) byte       MoverPitch;
...
// for MV_SpringByTime MoverGlideType
var()	float			MoverSpringTime;  	// how long it takes to spring in the end (in sec)
var()	float			MoverMaxAmplitude;	// how far it could spring (in percents)
var()	byte   			MoverFluctuations;	// how many fluctuations it could make		

var		vector			HitPosition;
...
{
	local PlayerPawn PPHarry;

	Super.Tick(Delta);

	// if this mover could not corral Harry, do nothing
	if(!bCorralMover)
		return;

	PPHarry = Level.PlayerHarryActor;

	if(!bCorraledFlag && (PPHarry.Base == Self) && MoverMoves())
	{
		PPHarry.fSavedGroundSpeed = PPHarry.GroundSpeed;
		PPHarry.GroundSpeed = 0.0;
		PPHarry.bCorraledByMover = true;

		bCorraledFlag = true;

		Log( "********* Harry is on top of the moving Mover **************" );
//		PPHarry.ClientMessage("********* Harry is on top of the moving Mover **************");
	}

	if(bCorraledFlag && !MoverMoves())
	{
		PPHarry.GroundSpeed = PPHarry.fSavedGroundSpeed;
		PPHarry.bCorraledByMover = false;

		bCorraledFlag = false;

		Log( "********* Mover Stoped after beeing corraled **************" );
//		PPHarry.ClientMessage("********* Mover Stoped after beeing corraled **************");
	}													   
}
...
final function InterpolateTo( byte NewKeyNum, float Seconds )
{
	...
	else
	{
		...
		// if mover rotates from the last key frame to the first one.
		// and we want to keep the same rotation direction, do a trick
		if(bKeepRotationDirection)
		{
			if( (NewKeyNum == 0) && (KeyNum == NumKeys - 1) )
			{
				if(KeyRot[0].Yaw < KeyRot[KeyNum].Yaw)
					OldRot.Yaw = Rotation.Yaw - 65536;
			}			
		}
	}
...
function FinishedClosing()
{
	...
	PlaySound( ClosedSound, SLOT_None, MoverVolume/128.0, , MoverRadius, MoverPitch/64.0);
...
function FinishedOpening()
{
	...
	// Update sound effects.
	if( ( Location==OldPos ) && ( Rotation==OldRot ) )
		// Did not move.
		playSound(FailSound, SLOT_None, MoverVolume/128.0, , MoverRadius, MoverPitch/64.0);
	else
		PlaySound( OpenedSound, SLOT_None, MoverVolume/128.0, , MoverRadius, MoverPitch/64.0);
...
function DoClose()
{
	...
	if(MoveTimes[KeyNum] > 0)
		InterpolateTo( Max(0,KeyNum-1), MoveTimes[KeyNum] );
	else
...
function bool EncroachingOn( actor Other )
{
	...
	if( (MoverEncroachType == ME_StopWhenEncroach) || (MoverEncroachType == ME_ReallyStopWhenEncroach) )
	{
		Leader.MakeGroupStop();
		return true;
	}
	else if( (MoverEncroachType == ME_ReturnWhenEncroach) || (MoverEncroachType == ME_ReallyReturnWhenEncroach) )
	{
	...
	else if( (MoverEncroachType == ME_CrushWhenEncroach) || (MoverEncroachType == ME_ReallyCrushWhenEncroach) )
	{
		// Kill it.
		Other.KilledBy( Instigator );
		return false;
	}
	else if( (MoverEncroachType == ME_IgnoreWhenEncroach) || (MoverEncroachType == ME_ReallyIgnoreWhenEncroach) )
	{
		// Ignore it.
		return false;
	}
...
defaultproperties
{
	 ...
	 bCorraledFlag=false
	 MoverSpringTime=1.0
	 MoverMaxAmplitude=0.1
	 MoverFluctuations=1	
	 bKeepRotationDirection=false
