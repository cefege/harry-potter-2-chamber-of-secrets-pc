...
var	bool		bCorraledByMover;
var	float 		fSavedGroundSpeed;
...
var float aForMove;
var travel bool bShowLoadingScreen;

//-----------------------------------------------------------------------------
// Game State

var				int							TotalGameStateTokens;	// Total number of GameStates
var				int							GameStateTokenLen;		// Length of a single GameState token ( set in defaultproperties )
var (GameState) const editconst string		GameStateMasterList;	// Master list of GameStates 
var (GameState) travel string				CurrentGameState;		// Absolute Current GameState

// --- Lumos Specific
// The engine will look at bLumosOn and fLumosRadius to determine if it should 
// calculate the lumos surface alpha values.
var bool	bLumosOn;	
...
// Call this function for all save games.  It will call the native ConsoleCommand("SaveGame X")
// and derived player class can override and do special stuff when game is saved.
function SaveGame( optional int iSaveGameNum )
{
	//NOTE: iSaveGameNum != SLOT Number 
	// (slots are just directories, the save game Number that the front end will always look for is zero )
	
	// Make sure we are not in slomo
	SloMo(1.0);
	
	ClientMessage( "Taking Thumbnail Screenshot - Save" $iSaveGameNum $".bmp" );

	// Save a Snapshot (thumbnail) of the current game
	Thumbnail( 120, 90, "Save" $iSaveGameNum $".bmp" );
	
	// Queue up a save game.
	ClientMessage( "Saving game..." );

    //ConsoleCommand("SaveGame " $iSaveGameNum );
	bQueuedToSaveGame = true;
}
...
// You can specify a filename for the screenshot, if you do then the engine will save it in your savePathDir.
// Otherwise it will save the screenshot in the userDir.
exec function SShot( optional string FileName )
{
	local float b;
	...
	ConsoleCommand("shot " $FileName );
	ConsoleCommand("set ini:Engine.Engine.ViewportManager Brightness "$string(B));
	ConsoleCommand("flush");
}

// Thumbnail
// - A reduced image of a graphic or document page, used in order to view multiple images on a screen simultaneously. 
//
// iShrinkFactor - The larger the number the smaller the image, ( 1 = 1/1, 2 = 1/2, 3 = 1/3, 4 = 1/4'th of the Original image size)
exec function Thumbnail( int iResX, int iResY, string FileName )
{
	local float b;
	b = float(ConsoleCommand("get ini:Engine.Engine.ViewportManager Brightness"));
	ConsoleCommand("set ini:Engine.Engine.ViewportManager Brightness 0.65");
	ConsoleCommand("flush");
	ConsoleCommand("thumbnail x=" $iResX $" y=" $iResY );
	ConsoleCommand("saveThumbnail " $FileName ); // save our snapShot as a thumbnail (which won't resize the snap to fit a texture )
	ConsoleCommand("set ini:Engine.Engine.ViewportManager Brightness "$string(B));
	ConsoleCommand("flush");
}
...
//************************************************************************
	...
	//ClientMessage("bStrafe:"$bStrafe$"  aLookUp:"$aLookUp);
	if( bStrafe!=0 )
	{
		// Strafe.
	...
	SmoothMouseY = AbsSmoothY;
		...
		//ClientMessage("bStrafe:"$bStrafe$" aStrafe:"$aStrafe$" aBaseX:"$aBaseX$" aTurn:"$aTurn$" aMouseX:"$aMouseX);
	//This adds a bit of turn to harry when he's strafing.  Makes him slightly "circle around the boss" while strafing.
	SmoothMouseX += -aStrafe * TurnWhileStrafingMult();//0.15;  //negative so you face towards your foe

	// add keyboard turning to the mouse (this should be revised)
	SmoothMouseX += aBaseX;
	...
	{
		aForward += aBaseY; //If using the aForward Key, aBaseY will be zero.  If using the aBaseY key, aForward will be zero.
	}

	//aBaseY   has forward movement from the key that moves you forward AND aims up
	//aForward has forward movement from the key that moves you forward and DOES NOT aim up
	
	if( PlayerIsAiming() )
	{	
		aLookUp += aBaseY * 0.3;

		// See if we are aiming with the keyboard
		if( bKeyAltFire!=0  ||  !MoveWhileCasting() )
		{
			SmoothMouseY += 0.5 * aBaseY * FOVScale;
			bKeyboardLook = true;
			aForward	  = 0;
		}
	}
	...
	//If the player is captured, get rid of the mouse look stuff.
	if( InputIsDisabled() )
	{
		SmoothMouseX = 0;
		SmoothMouseY = 0;
...
event ViewFlash(float DeltaTime)
{
	...
	//ft: if the engine just runs the first map it loads (like when you run a map from the editor), it will send in
	// DeltaTime as 0.  This turns off the fade.  Make sure DeltaTime doesn't get that small.
	delta = FMax(0.001, delta);
...
function ViewShake(float DeltaTime)
{
	...
	verttimer += DeltaTime; //shake view
	if( verttimer < shaketimer )
	{
		scale = (shaketimer-verttimer) / shaketimer;
		r.yaw =   RandRange( -maxshake, maxshake ) * scale;
		r.pitch = RandRange( -maxshake, maxshake ) * scale;
		r.roll =  RandRange( -shakemag, shakemag ) * scale;
	}
...
//********************************************************************************************
// Lumos functions
function bool InLumosRadius( vector pos )
{
	return (vsize(location - pos) < fLumosRadius);
}
...
function string GetGameStateMasterListToken( int index )
{
	// error checking
	if( index > TotalGameStateTokens )
		return "";
	
	return Mid( GameStateMasterList, index*(GameStateTokenLen+1), GameStateTokenLen );
}

// false == error, true == success
function bool SetGameState( string NewGameState )
{
	// Force caps, so we don't care about upper/lower case letters
	NewGameState = Caps( NewGameState );
	
	// Make sure this token is the correct length
	if( Len( NewGameState ) != GameStateTokenLen )
	{
		ClientMessage( "!E!R!R!O!R! GameState " $NewGameState $" is not a valid GameState in the *GameStateMasterList*!!! (its length is incorrect)" );
		log( "!E!R!R!O!R! GameState " $NewGameState $" is not a valid GameState in the *GameStateMasterList*!!! (its length is incorrect)" );
		return false; // error
	}
	
	// We already have this as our CurrentGameState
	if( CurrentGameState ~= NewGameState )
	{
		ClientMessage( "!O!K! GameState " $NewGameState $" IS the current GameState!" );
		log( "!O!K! GameState " $NewGameState $" IS the current GameState!" );
		return true; // ok
	}
	
	// Test our new gameState agenst our master list of GameStates
	if( InStr( GameStateMasterList, NewGameState ) >= 0 )
	{
		//DEBUG
		ClientMessage("!*!*!*!* Changing currentGameState to " $NewGameState $"!" );
		log("!*!*!*!* Changing currentGameState to " $NewGameState $"!" );
		
		// Set our new gamestate ( to be applied when we load a level )
		CurrentGameState = NewGameState;
		return true; // success
	}
	
	//DEBUG
	ClientMessage( "!E!R!R!O!R! GameState " $NewGameState $" is not a valid GameState in the *GameStateMasterList*!!! (its not in the list)" );
	log( "!E!R!R!O!R! GameState " $NewGameState $" is not a valid GameState in the *GameStateMasterList*!!! (its not in the list)" );
	return false; // error
}
...
defaultproperties
{
	...
	// Make sure the GameStateMasterList is always using CAPS, we need this to get rid of case sensitivity with InStr().
	TotalGameStateTokens=25
	GameStateTokenLen=9
	GameStateMasterList="GSTATE000,GSTATE010,GSTATE020,GSTATE030,GSTATE035,GSTATE040,GSTATE045,GSTATE050,GSTATE060,GSTATE065,GSTATE070,GSTATE080,GSTATE090,GSTATE095,GSTATE100,GSTATE110,GSTATE115,GSTATE120,GSTATE130,GSTATE140,GSTATE145,GSTATE150,GSTATE160,GSTATE170,GSTATE180"
	...
	ObjectDetail=ObjectDetailVeryHigh
	bCorraledByMover=false
	fSavedGroundSpeed=0.0
	ConstantGlowFog=(X=0,Y=0,Z=0,W=0)
}
