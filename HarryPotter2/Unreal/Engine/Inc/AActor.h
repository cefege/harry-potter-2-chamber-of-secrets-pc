...
=============================================================================*/
	...
	void SerializePersistence( FArchive& Ar );
	...
	virtual FLOAT WorldLightRadius() const {return Max(DrawScale, 1.f) * 25.f * ((INT)LightRadius+1);}
	...
	UBOOL PlayAnim( FName Sequence, bool Loop = false, float Rate = 1.f, float TweenTime = -1.f, float MinRate = 0.f, EAnimType Type = AT_Replace, FName RootBone = NAME_None );
	UBOOL IsAnimating() const
	{
		return
			(AnimSequence!=NAME_None)
		&&	(AnimRate!=0.f || TweenRate!=0.f);
	}
	AActor* CreateAnimChannel( UClass* NewClass, EAnimType Type, FName RootBone, bool bTransient = false, bool bNotReplaceable = false );
	...
	void CheckHearSound(APawn* Hearer, INT Id, USound* Sound, FVector Parameters, FLOAT RadiusSquared, UBOOL Disable3D, UBOOL Loop);
