...
enum EViewportBlitFlags
{
	...
	BLIT_Vulkan        = 0x0200, // Vulkan rendering.
...
private:
	// Internal functions.
	void VARARGS WrappedPrint( ERenderStyle Style, INT& XL, INT& YL, UFont* Font, UBOOL Center, const TCHAR* Text, INT numChars=0, UBOOL bCursor=false );
};
...
enum ERenderType
{
	...
	REN_MatineeIP		= 19,	// Matinee - interpolation point editing.
	REN_MAX				= 21
};
...
enum EViewportShowFlags
{
	...
	SHOW_Bounds			= 0x00008000,	// Show joint bounding volumes.
	SHOW_Bones			= 0x00010000,	// Show actor skeletons.
};
...
class ENGINE_API UViewport : public UPlayer
{
	...
	TArray<FColor> snapBuff; // array used to take snapshots of the viewport
	INT snapU, snapV; // proportions of snap
	...
	UBOOL ScreenShot( const TCHAR* Filename );
	...
	virtual void DrawString( DWORD Flags, UFont* Font, INT& DrawX, INT& DrawY, const TCHAR* Text, const FPlane& Color ) {}
	virtual UFont* CreateNativeFont( const TCHAR* FontName, int Height ) { return NULL; }

	// virtual Functions for Smacker
	virtual UBOOL LoadSmackerMovie(const TCHAR* lpsz_MoviePath) {return false;}
	virtual UBOOL BlitNextSmackerFrame() {return true;}
	virtual UBOOL ShutdownSmacker() {return true;}
...
class ENGINE_API UClient : public UObject
{
	...
	static FLOAT GetEffectiveFovAngle( FLOAT AuthoredHFov, INT ViewportX, INT ViewportY, UBOOL bMaintainVerticalFOV );
	// UClient interface.
	...
	virtual FLOAT GetUIScale() const { return 1.f; }
};
