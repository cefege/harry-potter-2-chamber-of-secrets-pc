...
// A truecolor value.
//
#define GET_COLOR_DWORD(color) (color).Int4
...
public:
	...
	{
		return ((Int4&0xff)<<16) + (Int4&0xff00) + ((Int4&0xff0000)>>16);
	}
	_WORD HiColor565() const
	{
		return ((Int4&0xf8) << 8) + ((Int4&0xfC00) >> 5) + ((Int4&0xf80000) >> 19);
	}
	_WORD HiColor555() const
	{
		return ((Int4&0xf8) << 7) + ((Int4&0xf800) >> 6) + ((Int4&0xf80000) >> 19);
	}
	FVector Plane() const
	{
...
class ENGINE_API UTexture : public UBitmap
{
	...
	// Sounds.
	BYTE		FootstepSound;		// Footstep sound type.
	USound*		HitSound GCC_PACK(4);	// Sound when the texture is hit with a projectile.
	...
	INT DefaultLOD();
	UBOOL LoadFromSnap ( UViewport *, unsigned int Usize, unsigned int Vsize );
	UBOOL LoadFromBMP( FString BMPPath );
...
class ENGINE_API UFont : public UObject
{
	...
	UBOOL IsRemapped;
	FString FontName;			// If font is to be rendered natively.
	INT FontHeight;				// ditto.
	void* NativeFont;			// OS handle for native font.
...
{
	DECLARE_CLASS(UEngineTextureFactory,UFactory,0,Editor)
	UEngineTextureFactory();
