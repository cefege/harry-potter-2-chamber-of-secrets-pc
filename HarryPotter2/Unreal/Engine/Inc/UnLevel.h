...
public:
	...
	virtual void RemoveActor( AActor *Actor, bool bThorough = false )=0;
...
{
	// Bitflags.
	TRACE_BlockActors   = 0x01, // Check collision with actors that block.
	...
	TRACE_AllBlocking   = TRACE_BlockActors | TRACE_Level | TRACE_Movers,
	TRACE_AllColliding  = TRACE_BlockActors | TRACE_Movers | TRACE_Level | TRACE_Others,
	TRACE_AllEverything = TRACE_BlockActors | TRACE_Movers | TRACE_Level | TRACE_ZoneChanges | TRACE_Others,
};
...
class ENGINE_API ULevel : public ULevelBase
{
	...
	// --- HP2 GameState related functions
	void ScreenActorsByGameState	( void );
	bool SavePersistentActors		( const FString& MapName );
	bool LoadPersistentActors		( const FString& MapName );
