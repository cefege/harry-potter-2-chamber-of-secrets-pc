...
{ \
	...
	if( !Owner ) \
		if \
		(   !ClipTo( FPlane( 0.f, 0.f,-1.0f,0.1f-Box.Min.Z), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.f, 0.f,+1.0f,Box.Max.Z+0.1f), INDEX_NONE ) \
		||	!ClipTo( FPlane(-1.f, 0.f, 0.0f,0.1f-Box.Min.X), INDEX_NONE ) \
		||	!ClipTo( FPlane(+1.f, 0.f, 0.0f,Box.Max.X-0.1f), INDEX_NONE ) \
		||  !ClipTo( FPlane( 0.f,-1.f, 0.0f,0.1f-Box.Min.Y), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.f,+1.f, 0.0f,Box.Max.Y-0.1f), INDEX_NONE ) ) \
			goto NoBlock; \
	\
	...
	if( Nodes.Num() )
	{
		...
		else
		{
			...
			do
			{
				...
				IsFront = Node.Plane.TransformPlaneByOrtho(Coords).PlaneDot(Location) > 0.f;
...
struct FBoxLineCheckInfo : public FBoxCheckInfo
{
	...
	UBOOL ClipTo( const FPlane& Hull, INT Item )
	{
		...
		else if( (D0-D1) > +0.00001f      ) { if( T > T0 ) 
			{ T0 = T; LocalHit = Hull; if( Item >= 0 ) LocalItem = Item; } }
...
)
{
	...
	if( Nodes.Num() )
	{
		...
		else
		{
			...
			// Truncate by 0.5 world units.
			if( Trace.DidHit )
			{
				Hit.Time      = Clamp( Hit.Time - 0.5f/Trace.Dist, 0.f, 1.f );
