		...
		if( !Result )
		{
			...
			for( TFieldIterator<UProperty> It(Class); It; ++It )
				if( It->PropertyFlags & CPF_Input )
					Count++;
			Result = (FInputVarCache*)GCache.Create(CacheId,Item,appCheckedIntSize(sizeof(FInputVarCache)+static_cast<SIZE_T>(Count)*sizeof(UProperty*)));
			Result->Count = Count;
			for( TFieldIterator<UProperty> It(Class); It; ++It )
				if( It->PropertyFlags & CPF_Input )
...
void UInput::StaticInitInput()
{
	...
	for( INT i=0; i<IK_MAX; i++ )
	{
		if( InputKeys->Names(i)!=NAME_None )
		{
			...
			new(StaticClass(),Str+3,RF_Public)UStrProperty( EC_CppProperty, STRUCT_OFFSET(UInput,Bindings[i]), TEXT("RawKeys"), CPF_Config );
		}
	}
...
UBOOL UInput::Exec( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Str, TEXT("AXIS") ) )
	{
		...
		{
			FLOAT Speed=1.f, SpeedBase=0.f;
			Parse( Str, TEXT("SPEED="), Speed );
			Parse( Str, TEXT("SPEEDBASE="), SpeedBase );
			if( SpeedBase > 0.f )
			{
				*Axis += GetInputDelta() * SpeedBase * Speed;
			} else {
				if( GetInputAction() == IST_Axis )
				{
					*Axis += 0.01f * GetInputDelta() * Speed;
				}
...
{
	FString Command = FString::Printf(TEXT("%s Speed=%f"), *Bindings[iKey], Length);
...
void AActor::execGetCurrentKeyState( FFrame& Stack, RESULT_DECL )
{
	guardSlow(AActor::execGetCurrentKeyState);
	...
	if( Key < IK_MAX )
	{
		...
		UViewport* Viewport = PlayerPawn ? Cast<UViewport>(PlayerPawn->Player) : NULL;
		if( Viewport && Viewport->Input )
			ReturnValue = Viewport->Input->KeyDown(Key) != 0;
	}
	appMemcpy(Result, &ReturnValue, sizeof(ReturnValue));
