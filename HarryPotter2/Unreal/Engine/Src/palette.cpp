...
unsigned short *Histogram8888(DWORD  *data,
                              int            width,
                              int            height)
{
   int   i;
   ...
   i = width * height;
   qd = (unsigned short*) appMalloc(appCheckedAllocSize(static_cast<SIZE_T>(i) * sizeof(*qd)), TEXT("Histogram8888"));
...
//
//
unsigned short *HistogramX888(DWORD *data,
                              int width,
                              int height)
{
   int   i;
   ...
   i = width * height;
   qd = (unsigned short*) appMalloc(appCheckedAllocSize(static_cast<SIZE_T>(i) * sizeof(*qd)), TEXT("HistogramX888"));
#undef RDATA
#undef GDATA
#undef BDATA
                             ...
                             int height)
{
   ...
   i = width*height*3;
   qd = (unsigned short*) appMalloc(appCheckedAllocSize(static_cast<SIZE_T>(width) * static_cast<SIZE_T>(height) * sizeof(*qd)), TEXT("Histogram888"));
   j=0;
   k=0;
   while(j<i)
      {
...
DWORD  ascale = 400;
DWORD  rscale = 200;      //400
DWORD  gscale = 300;
DWORD  bscale = 100;      //500
                                ...
                                int  pal_format)
{
   ...
   INT  adiff,rdiff,gdiff,bdiff,diff,mindiff,best;
   INT  as,rs,gs,bs;
                       ...
                       int             generate_mipmaps)
{
   ...
   // Clear the histogram
   appMemset(histogram, 0, 65536 * sizeof(DWORD));
   colour_count = 0;

   // If we will need to mipmap, clear the colour mapping data
   if(generate_mipmaps)
      appMemset(in_pal, 0, 65536 * sizeof(DWORD));
   ...
   switch(srcformat)
      {
      case  ARGB_8888:
         quantise_data = Histogram8888((DWORD*) srcdata,width,height);
         alpha_data = true;
         ...
         break;
      case  XRGB_8888:
         quantise_data = HistogramX888((DWORD*) srcdata,width,height);
         hist_format = RGB_565;
         amul = 0;
         rmul = 31;
         gmul = 63;
