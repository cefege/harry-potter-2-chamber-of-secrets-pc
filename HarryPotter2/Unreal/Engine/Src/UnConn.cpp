...
UBOOL UNetConnection::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	{
		Ar.Logf( TEXT(" %i"), appRound(BestLag*1000.f) );
		return 1;
	}
	else if( ParseCommand(&Cmd,TEXT("GETLOSS")) )
	{
		Ar.Logf( TEXT(" %i"), appRound(InLoss) );
		return 1;
	}
...
void UNetConnection::ReceivedRawPacket( void* InData, INT Count )
{
	...
	debugfSlow( NAME_DevNetTraffic, TEXT("%03i: Received %i"), appRound(appSeconds().GetFloat()*1000)%1000, Count );
	...
	if( Out.GetNumBits() || Driver->Time-LastSendTime>Driver->KeepAliveTime )
	{
		...
		else if( PktLag )
		{
			if( !PktLoss || appFrand()*100.f>PktLoss )
			{
				...
				B.Data.Add( Out.GetNumBytes() );
				appMemcpy( &B.Data(0), Out.GetData(), Out.GetNumBytes() );
				B.SendTime = appSeconds() + (PktLag / 1000.f);
			}
		}
...
void UNetConnection::ReceivedPacket( FBitReader& Reader )
{
	...
	//debugf(TEXT("RcvdPacket: %i %i"),appRound(appSeconds()*1000)%1000,PacketId);
...
//				{
					LagAcc += (Driver->Time - OutLagTime[Index]) - FrameTime*0.5f;
					LagCount++;
//				}
			}
...
void UNetConnection::Tick()
{
	...
	CountedFrames++;
	if(CumulativeTime.GetFloat() > 1.f)
	{
		AverageFrameTime = CumulativeTime.GetFloat() / CountedFrames;
		CumulativeTime   = 0.f;
		CountedFrames    = 0;
	}
	...
	if( Driver->Time-StatUpdateTime>StatPeriod )
	{
		...
		if( Actor )
		{
			...
			Actor->bBadConnectionAlert = !InternalAck && ((BestLag>1.0 || CurrentNetSpeed * (1 - PktLoss)<2000) && ActorChannels.FindRef(Actor)) || InPackets < 2;
		}
	...
	QueuedBytes     -= appRound(DeltaBytes);
	FLOAT AllowedLag = 2.f * DeltaBytes;
	if( QueuedBytes < -AllowedLag )
		QueuedBytes = -appRound(AllowedLag);
