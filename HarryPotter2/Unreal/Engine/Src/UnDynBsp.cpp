...
template <class T> INT ExpandDb( TArray<T>& Array, INT Slack, INT MaxCount )
{
	...
	if( Add > 0 )
	{
		// Avoid wasteful Add/Shrink cycle.
		Array.Set( Array.Num()+Add );
		appMemzero( &Array(Index), Add*sizeof(T) );
	}
	else
		Array.Shrink();
	return Index;
...
)
{
	...
	check(Helper.Num()==FreeLinks.Num());
	INT Index;
	if( FirstLink!=INDEX_NONE )
	{
		Index = FirstLink;
		const INT HelperIndex = Index - FirstMaster;
		check(!Helper(HelperIndex));
		FirstLink = FreeLinks(HelperIndex);
	}
	else
	{
		Index = Master.AddZeroed();
		Helper.AddZeroed();
		FreeLinks.AddItem(INDEX_NONE);
	}
	const INT HelperIndex = Index - FirstMaster;
	Helper(HelperIndex) = HelperValue;
	FreeLinks(HelperIndex) = INDEX_NONE;
	return Index;
	unguard;
}
...
{
	const INT HelperIndex = Index - FirstMaster;
	check(HelperIndex>=0 && HelperIndex<Helper.Num());
	check(Helper.Num()==FreeLinks.Num());
	Helper(HelperIndex) = T();
	FreeLinks(HelperIndex) = FirstLink;
	FirstLink = Index;
}
...
)
{
	...
	{
		INT IsFlipped  = (Node->Plane|Parent->Plane)<0.f;
...
void FMovingBrushTracker::FlushActorBrush( AMover* Actor )
{
	...
	while( iNode != INDEX_NONE )
	{
		...
		for( DWORD j=0; j<Node->NumVertices; j++ )
			FreeThang( VertPool[j].pVertex, iFirstRovingPoint, PointOwners, PointFreeLinks, iFreePoints );
