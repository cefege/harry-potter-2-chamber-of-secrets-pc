...
UBOOL ULevel::DestroyActor( AActor* ThisActor, UBOOL bNetForce )
{
	...
	if( Hash )
	{
		if( ThisActor->bCollideActors )
			Hash->RemoveActor( ThisActor, true );
	...
	for( INT iActor=0; iActor<Actors.Num(); iActor++ )
	{
		...
		if( Other )
		{
			if( Other->TickParent==ThisActor )
				Other->TickParent = NULL;

			if( Other->TickParent2==ThisActor )
				Other->TickParent2 = NULL;
...
void ULevel::AdjustSpot( FVector& Adjusted, FVector TraceDest, FLOAT TraceLen, FCheckResult& Hit )
{
	...
	if( Hit.Time < 1.f )
		Adjusted = Adjusted + Hit.Normal * (1.05f - Hit.Time) * TraceLen;
}
...
)
{
	...
	FLOAT TraceLen = Extent.Size() + 2.f;
...
UBOOL ULevel::FarMoveActor( AActor* Actor, FVector DestLocation,  UBOOL test, UBOOL bNoCheck )
{
	...
	if( result )
	{
		if( !test && (GetLevelInfo()->NetMode != NM_Client) )
		{
...
UBOOL DoMultiLineCheck(AActor* Actor)
{
	AMover* Mover;

	// if this is not a mover, do check
 	if(!Actor->IsA(AMover::StaticClass()) )
		return 1;

	Mover  = (AMover*)Actor;

	if(Mover->MoverEncroachType == ME_StopWhenEncroach)
		return 1;

	if(Mover->MoverEncroachType == ME_ReturnWhenEncroach)
		return 1;

	if(Mover->MoverEncroachType == ME_CrushWhenEncroach)
		return 1;

	if(Mover->MoverEncroachType == ME_IgnoreWhenEncroach)
		return 1;
...
)
{
	...
	FLOAT TestAdjust	   = Actor->IsMovingBrush() ? 0.f : 2.f;
	...
	if( (Actor->bCollideActors || Actor->bCollideWorld) && Delta!=FVector(0,0,0) )
	{
		// Check collision along the line.
		FBox Box = Actor->GetPrimitive()->GetCollisionBoundingBox(Actor, true);
		FVector Location = (Box.Min + Box.Max) * 0.5f;
		FVector Extent = (Box.Max - Box.Min) * 0.5f;

		// Subtract the trace margin, for flush movement.
		if( Actor->IsMovingBrush() )
			Extent -= FVector(0.51f);

		if( DoMultiLineCheck(Actor) )
		{
...
//
// If it is just rotation (location was not changed), it could cost a problem
// That is why we calculate so called "Delta from Rotation" as well,
//
FVector FindDeltaFromRotationYaw(FVector origin, FVector point, int angle)
{
	FVector  retVector;

	float A, B, x, y, length;
	float sinAlpha, cosAlpha;
	float sinBetha, cosBetha;

	A = point.X - origin.X;
	B = point.Y - origin.Y;

	length = appSqrt(A * A + B * B);

	cosBetha = A / length;
	sinBetha = B / length;

	sinAlpha = appSin(angle / (32768.0f / PI));
	cosAlpha = appCos(angle / (32768.0f / PI));

	x = length * (cosAlpha * cosBetha - sinAlpha * sinBetha) + origin.X;
	y = length * (sinAlpha * cosBetha + cosAlpha * sinBetha) + origin.Y;
...
FVector FindDeltaFromRotationRoll(FVector origin, FVector point, int angle)
{
	FVector  retVector;

	float A, B, y, z, length;
	float sinAlpha, cosAlpha;
	float sinBetha, cosBetha;
	...
	sinAlpha = appSin(-angle / (32768.0f / PI));		// Orientation problem
	cosAlpha = appCos(-angle / (32768.0f / PI));		// Orientation problem

	y = length * (cosAlpha * cosBetha - sinAlpha * sinBetha) + origin.Y;
	z = length * (sinAlpha * cosBetha + cosAlpha * sinBetha) + origin.Z;
...
FVector FindDeltaFromRotationPitch(FVector origin, FVector point, int angle)
{
	FVector  retVector;

	float A, B, x, z, length;
	float sinAlpha, cosAlpha;
	float sinBetha, cosBetha;
...
FVector FindDeltaFromRotation(FVector origin, FVector point, int yaw, int roll, int pitch)
{
	FVector v1, v2, v3;

	v1 = FindDeltaFromRotationRoll (origin, point, roll );
	v2 = FindDeltaFromRotationPitch(origin, v1,    pitch);
	v3 = FindDeltaFromRotationYaw  (origin, v2,    yaw  );
	...
	for( Test = FirstHit; Test!=NULL; Test=Test->GetNext() )
	{
		...
		&&	Actor->IsBlockedBy( Test->Actor ) )
		{
			if ( Actor->IsMovingBrush() && !Test->Actor->IsMovingBrush() ) 
			{
				...
				FVector deltaFromRotation = FindDeltaFromRotation(Actor->Location, Test->Actor->Location, 
					TestRotation.Yaw   - Actor->Rotation.Yaw,
					TestRotation.Roll  - Actor->Rotation.Roll,
					TestRotation.Pitch - Actor->Rotation.Pitch);

				FVector MoveDir = TestLocation - Actor->Location + 10 * deltaFromRotation;	// multiply by 10 to make it more visisble
...
)
{
	...
	for( Check = FirstHit; Check!=NULL; Check=Check->GetNext() )
	{
		if( !SourceActor || !SourceActor->IsOwnedBy( Check->Actor ) )
		{
			...
			{
				// Distinguish based on blocking status.
				if( SourceActor && SourceActor->IsBlockedBy(Check->Actor) )
				{
					if( TraceFlags & TRACE_BlockActors )
						break;
				}
				else
				{
...
)
{
	...
	if( (bCheckActors || LevelInfo) && Hash )
	{
		...
		{
			if( !bCheckActors && !Link->Actor->IsA(AMover::StaticClass()) )
				// Only interested in Mover actors.
				continue;
