...
#include "UnRender.h"
#include "UnMesh.h"
#if ASM3DNOW && defined(_MSC_VER) && defined(_M_IX86)
#include "Amd3d.h"
#endif
...
UMesh::UMesh()
{
	...
	RotOrigin		 = FRotator(0.f,0.f,0.f);
...
void UMesh::SetScale( FVector NewScale )
{
	Scale = NewScale;
}
...
//
FCoords UMesh::GetRenderBoundingBox( const AActor* Owner, UBOOL Exact )
{
	...
	if( Seq && Owner->AnimFrame>=0.0f )
	{
		// Animating, so use bound enclosing two frames' bounds.
		INT iFrame = appFloor((Owner->AnimFrame+1.0f) * Seq->NumFrames);
	...
	FCoords Coords = FCoords(-Origin) << (Scale*Owner->DrawScale) << RotOrigin
		<< Owner->PrePivot << Owner->Rotation << Owner->Location;
	return Bound << Coords;
	unguardobj;
}

// Override UBoxPrim behaviour to incorporate scale and origin.
FBox UMesh::GetCollisionBoundingBox( const AActor* Owner, bool bWorld ) const
{
	// Transform Bound by owner's scale and origin.
	FVector BoxScale = Scale*Owner->DrawScale;
	FBox Bound( BoxScale*(BoundingBox.Min - Origin), BoxScale*(BoundingBox.Max - Origin) );
	if( bWorld )
	{
...
FVector UMesh::GetCollisionExtent( const AActor* Owner ) const
{
	FBox Bound = GetCollisionBoundingBox( Owner, false );
	return FVector( Max( -Bound.Min.X, Bound.Max.X ),
					Max( -Bound.Min.Y, Bound.Max.Y ),
					Max( -Bound.Min.Z, Bound.Max.Z ) );
}
...
//
inline void UMesh::AMD3DGetFrame
(
	FVector*	ResultVerts,
	INT			Size,
	FCoords		Coords,
...
)
{
	...
	if( Mem==NULL || *(UMesh**)Mem!=this )
	{
		...
		Mem = GCache.Create( CacheID, Item, appCheckedIntSize(sizeof(UMesh*) + sizeof(FLOAT) + sizeof(FName) + static_cast<SIZE_T>(FrameVerts) * sizeof(FVector)) );
		WasCached = 0;
	}
	...
	Coords                  = Coords * (Owner->Location + Owner->PrePivot) * Owner->Rotation * RotOrigin * FScale(Scale * Owner->DrawScale,0.0,SHEER_None);
	const FMeshAnimSeq* Seq = GetAnimSeq( Owner->AnimSequence );

	// Transform all points into screenspace.
	if( Owner->AnimFrame>=0.0f || !WasCached )
	{
	...
	else
	{
		...
		FLOAT StartFrame = Seq ? (-1.0f / Seq->NumFrames) : 0.0f;
		INT iFrameOffset = Seq ? Seq->StartFrame * FrameVerts : 0;
		FLOAT Alpha = 1.0f - Owner->AnimFrame / CachedFrame;
		if( CachedSeq!=Owner->AnimSequence || Alpha<0.0f || Alpha>1.0f)
		{
...
)
{
	...
	QWORD CacheID    = MakeCacheID( CID_TweenAnim, Owner, NULL );
	BYTE* Mem        = GCache.Get( CacheID, Item );
	SIZE_T HeaderSize = sizeof(UMesh*) + sizeof(FLOAT) + sizeof(FName);
	SIZE_T AlignHint = Align(HeaderSize, 16) - HeaderSize; // GM: AlignHint
	if( Mem==NULL || *(UMesh**)Mem!=this )
	{
		if( Mem != NULL )
		{
		...
		Mem = GCache.Create( CacheID, Item, appCheckedIntSize(HeaderSize + AlignHint + static_cast<SIZE_T>(FrameVerts) * sizeof(FVector)) );
		WasCached = 0;
	}
	UMesh*& CachedMesh  = *(UMesh**)Mem; Mem += sizeof(UMesh*);
	FLOAT&  CachedFrame = *(FLOAT *)Mem; Mem += sizeof(FLOAT );
	...
	else
	{
		...
		FLOAT StartFrame = Seq ? (-1.0f / Seq->NumFrames) : 0.0f;
		INT iFrameOffset = Seq ? Seq->StartFrame * FrameVerts : 0;
		FLOAT Alpha = 1.0f - AnimOwner->AnimFrame / CachedFrame;
		if( CachedSeq!=AnimOwner->AnimSequence || Alpha<0.0f || Alpha>1.0f)
		{
...
FCoords UMesh::GetBoneCoords( AActor* Owner, int Bone ) const
{
	return FCoords(-Origin) / FScale(Scale * Owner->DrawScale, 0.0f, SHEER_None) / RotOrigin
		/ Owner->Rotation / Owner->PrePivot / Owner->Location;
}
