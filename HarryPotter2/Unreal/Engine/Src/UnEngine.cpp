...
void UEngine::Init()
{
	...
	// Subsystems.
	FURL::StaticInit();
	GEngineMem.Init( 65536, TEXT("GEngineMem") );
...
void UEngine::Serialize( FArchive& Ar )
{
	...
	Super::Serialize( Ar );
	Ar << Cylinder << OCylinder << OvalCylinder << OOvalCylinder << Box << Client << Render << Audio;
