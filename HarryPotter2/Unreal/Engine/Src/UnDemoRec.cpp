...
UBOOL UDemoRecDriver::InitListen( FNetworkNotify* InNotify, FURL& ConnectURL, FString& Error )
{
	...
	GetLevel()->WelcomePlayer( Connection, (GetLevel()->GetLevelInfo()->NetMode == NM_Client || GetLevel()->GetLevelInfo()->NetMode == NM_Standalone) ? TEXT("CLIENTDEMO") : TEXT("SERVERDEMO") );
		...
		for( ; ; )
		{
			...
			*FileAr << ServerPacketTime;
			...
			if(!NoFrameCap && !TimeBased && ServerPacketTime > Time)
			{
				...
				FTime t = appSeconds() + (ServerPacketTime - Time);
				while(t>appSeconds());
			}
			*FileAr << PacketBytes;
