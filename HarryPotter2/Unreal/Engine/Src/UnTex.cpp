...
//
void UTexture::Lock( FTextureInfo& TextureInfo, FTime CurrentTime, INT LOD, URenderDevice* RenDev )
{
	guard(UTexture::Lock);
	...
	// Fixup textures ripped of their mips, for dedicated server
	if( !bParametric && WhichMips.Num()==0 )
	{
		guard(RippedTextureFixup);
		WhichMips = Mips;
		Init( USize, VSize );
		unguard;
	}
...
enum EBitmapCompression
{
 ...
 // Sto: the following two fns do not work properly yet.
// May never now be needed. Based on code taken
// from undying that didnt seem to work

UBOOL UTexture::LoadFromSnap ( UViewport * viewport, unsigned int USize, unsigned int VSize)
{
	Format=TEXF_RGBA8;
	Init (USize, VSize);

	unsigned int srcU = (USize < viewport->snapU) ? USize : viewport->snapU;
	unsigned int srcV = (VSize < viewport->snapV) ? VSize : viewport->snapV;

	unsigned int srcDispX = (srcU < viewport->snapU) ? ((viewport->snapU-srcU)/2) : 0;

	for (unsigned int y=0; y<srcV; ++y)
	{
		BYTE* DestPtr = &Mips(0).DataArray(y * USize * 4);
		FColor* IntDstPtr = (FColor*)DestPtr;

		for (unsigned int x=0; x<srcU; ++x)
			IntDstPtr[x] = viewport->snapBuff( y*viewport->snapU + x + srcDispX );
	}
/*
	// Set bottom edges black or transparent
	for (y=srcV; y<VSize; ++y)
	{
		BYTE* DestPtr = &Mips(0).DataArray(y * USize * 4);
		FColor* IntDstPtr = (FColor*)DestPtr;
...
UBOOL UTexture::LoadFromBMP( FString BMPPath )
{
	TArray<BYTE> NewBMP;

	if ( appLoadFileToArray( NewBMP, *BMPPath ) )
	{
		NewBMP.AddItem(0);

		const BYTE* Buffer = &NewBMP( 0 );
		const BYTE* BufferEnd = Buffer+NewBMP.Num()-1;
		...
		if( (Length>=(INT)(sizeof(FBitmapFileHeader)+sizeof(FBitmapInfoHeader))) && Buffer[0]=='B' && Buffer[1]=='M' )
		{
			// This is a .bmp type data stream.

			// Calculate size of Texture.
			// Proportions must always be powers of 2
			
			int widthFactor =0;
			INT destWidth = bmhdr->biWidth;
			for (INT Width = destWidth; Width; Width>>=1)
				widthFactor ++;
			destWidth = static_cast<INT>(appPow(2, widthFactor));
			
			int heightFactor =0;
			INT destHeight = bmhdr->biHeight;
			for (INT Height = destHeight; Height; Height>>=1)
				heightFactor ++;
			destHeight = static_cast<INT>(appPow(2, heightFactor));

			// initialise this bmp, (creates a single mipmap of the right size)
			Format=TEXF_RGBA8;
			Init (destWidth, destHeight);

			if( bmhdr->biCompression != BCBI_RGB )
			{
				debugf( TEXT("RLE compression of BMP images not supported") );
				return 0;
			}
			if( bmhdr->biPlanes==1 && bmhdr->biBitCount==24 )
			{
				...
				for( unsigned int y=0; y<bmhdr->biHeight; y++ )
				{
					BYTE* DestPtr = &Mips(0).DataArray((bmhdr->biHeight - 1 - y) * destWidth * 4);
					BYTE* SrcPtr = (BYTE*) &Ptr[y * bmhdr->biWidth * 3];

					IntDstPtr = (INT*)DestPtr;
					IntSrcPtr = (INT*)SrcPtr;

					for( unsigned int x=0; x<bmhdr->biWidth; x++ )
					{
#if 0
						*DestPtr++ = *SrcPtr++;
						*DestPtr++ = *SrcPtr++;
						...
						*DestPtr++ = 0xFF;
#else
						*IntDstPtr = ((*IntSrcPtr) & 0xffffff) | 0xff000000;
						SrcPtr += 3;
						IntSrcPtr = (INT*)SrcPtr;
						IntDstPtr++;
#endif
			...
			{
				debugf(TEXT("BMP uses an unsupported format (%i/%i)"), bmhdr->biPlanes, bmhdr->biBitCount );
				return false;
			}
		}	
	}
...
// Sto: have copied the following code from the editor dll,
// as we need to import a texture in the game, for the bitmap images to appear

void UEngineTextureFactory::StaticConstructor()
{
	guard(UEngineTextureFactory::StaticConstructor);
...
UEngineTextureFactory::UEngineTextureFactory()
{
	guard(UEngineTextureFactory::UEngineTextureFactory);
	unguard;
}

static bool IsMasked( UTexture* Texture )
{
	if( Texture->Palette && Texture->Palette->Colors.Num() )
	{
		// Look for standard masking color.
		if( Texture->Palette->Colors(0) == FColor(255,0,255,255) )
		{
			// Look for any 0 pixels.
			FMipmap& Mip = Texture->Mips(0);
			INT Size = Mip.USize*Mip.VSize;
			for_count( i, Size )
				if( Mip.DataArray(i) == 0 )
					return true;
		}
	}
	return false;
...
TCHAR* GFile=NULL;
UObject* UEngineTextureFactory::FactoryCreateBinary
(
	UClass*				Class,
	UObject*			InParent,
	FName				Name,
...
{
	guard(UEngineTextureFactory::FactoryCreate);
...
IMPLEMENT_CLASS(UEngineTextureFactory);

// Sto: End code ripped wholesale from Editor dll
