...
void UActorChannel::ReceivedBunch( FInBunch& Bunch )
{
	...
	if( Actor==NULL )
	{
		...
		if( InActor==NULL )
		{
			...
			if ( !InActor )
			{
				GWarn->Logf(TEXT("Actor not spawned for new channel"));
				Broken = 1;
				return;
			}
		}
...
void UActorChannel::ReplicateActor()
{
	...
	LastRep            = Actor->GetOptimizedRepList( CompareBin, &Retirement(0), Reps, Connection->PackageMap, NumReps );
	NumReps++;
					...
					if( Info.Guid==Guid && Info.URL!=TEXT("") )
					{
						...
						if( Connection->Driver->Notify->NotifySendingFile( Connection, Guid ) )
						{
							...
							if( SendFileAr )
							{
								...
								debugf( NAME_DevNet, LocalizeProgress("NetSend"), SrcFilename );
								PackageIndex = i;
								return;
							}
						}
