...
public:
	...
	void RemoveActor( AActor *Actor, bool bThorough = false );
...
//
void FCollisionHash::RemoveActor( AActor* Actor, bool bThorough )
{
	...
	if( bThorough )
	{
		// Search entire hash array, to find scraps left by any sloppy move code.
		for( INT i=0; i<NUM_BUCKETS; i++ )
		{
			FCollisionLink** Link = &Hash[i];
			while( *Link )
			{	
				if( ((*Link)->Actor != Actor) && (*Link)->Actor )
				{
					Link = &(*Link)->Next;
				}
				else
	...
	{
		// Search only where the actor should be. 
		INT X0,Y0,Z0,X1,Y1,Z1;
		GetActorExtent( Actor, X0, X1, Y0, Y1, Z0, Z1 );
		for( INT X=X0; X<=X1; X++ )
		{
		...
		for( INT Y=Y0; Y<=Y1; Y++ )
		{
			for( INT Z=Z0; Z<=Z1; Z++ )
			{
				...
				for( FCollisionLink* Link = GetHashLink( X, Y, Z, iLocation ); Link; Link=Link->Next )
				{
					// Skip if we've already checked this actor.
					if
					(	Link->Actor //!!FIXME - hack fix for crash - what is causing this?
					&&	Link->Actor->CollisionTag != CollisionTag
					&&	Link->iLocation           == iLocation )
					{
						// Check collision.
