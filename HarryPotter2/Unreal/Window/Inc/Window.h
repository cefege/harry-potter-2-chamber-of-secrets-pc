...
{
public:	// FControlSnoop interface.
...
class WINDOW_API WButton : public WControl
{
	...
	void OnDrawItem( DRAWITEMSTRUCT* Item )
	{
		...
		if( bPressed )
		{
			HPEN penOld, pen = CreatePen( PS_SOLID, 1, RGB(150,150,150) );
...
/*		else
		{
			HPEN penOld,
				...
				penGrey  = CreatePen( PS_SOLID, 1, RGB(150,150,150) ),
				penDark  = CreatePen( PS_SOLID, 1, RGB( 80, 80, 80) );
			
			RECT rc = R;
			rc.right--;
			rc.bottom--;
			penOld = (HPEN)SelectObject( Item->hDC, penWhite );
			::MoveToEx( Item->hDC, rc.right, rc.top, NULL );
			::LineTo( Item->hDC, rc.left, rc.top );
			::LineTo( Item->hDC, rc.left, rc.bottom );
			
			(HPEN)SelectObject( Item->hDC, penDark );
			::LineTo( Item->hDC, rc.left, rc.bottom );
			::LineTo( Item->hDC, rc.right, rc.bottom );
			::LineTo( Item->hDC, rc.right, rc.top-1 );
			
			(HPEN)SelectObject( Item->hDC, penGrey );
			::MoveToEx( Item->hDC, rc.left+1, rc.bottom-1, NULL );
			::LineTo( Item->hDC, rc.right-1, rc.bottom-1 );

			::MoveToEx( Item->hDC, rc.right-1, rc.top, NULL );
			::LineTo( Item->hDC, rc.right-1, rc.bottom );
			SelectObject( Item->hDC, penOld );
...
/*----------------------------------------------------------------------------
	WListView. from warfare. needed for debugger
-----------------------------------------------------------------------------*/
...
class WINDOW_API WCoolButton : public WButton
{
	...
	WCoolButton()
	{}
	WCoolButton( WWindow* InOwner, INT InId=0, FDelegate InClicked=FDelegate(), DWORD InFlags=CBFF_ShowOver|CBFF_DimAway, DWORD InColor=0 )
	: WButton( InOwner, InId, InClicked )
	, hIcon( NULL )
	, FrameFlags( InFlags )
	, TextColor( InColor )
...
//#ifndef JAPANESE
	...
	// Draw edges onto button frame ( helper function for DrawItem() )
	void DrawEdges( DRAWITEMSTRUCT* Item )
	{		
		...
		if( GlobalCoolButton == this )
		{
			if( FrameFlags & CBFF_ShowOver )
				DrawEdge( Item->hDC, &Item->rcItem, bPressed||bDisabled?EDGE_SUNKEN:EDGE_RAISED, BF_RECT );
		}
		else
		{
			if( FrameFlags & CBFF_DimAway )
				DrawEdge( Item->hDC, &Item->rcItem, bPressed||bDisabled?BDR_SUNKENINNER:BDR_RAISEDINNER, BF_RECT );
			else if( FrameFlags & CBFF_ShowAway )
				DrawEdge( Item->hDC, &Item->rcItem, bPressed||bDisabled?EDGE_SUNKEN:EDGE_RAISED, BF_RECT );
		}
	}

	// Draw text onto button ( helper function for DrawItem() )
	void DrawItemText( DRAWITEMSTRUCT* Item )
	{		
		UBOOL bPressed  = (Item->itemState&ODS_SELECTED)!=0 || bChecked;
		RECT  R		    = Item->rcItem;

		R.left += bPressed; R.right += bPressed; R.top += bPressed; R.bottom += bPressed;
		...
		if( FrameFlags & CBFF_UrlStyle )
		{
			R.left += 8;
			SetTextColor( Item->hDC, RGB(0,0,255) );
			OldFont = (HFONT)SelectObject( Item->hDC, hFontUrl );
		}
		else
		{
			OldFont = (HFONT)SelectObject( Item->hDC, hFontText );
		}
		SetBkMode( Item->hDC, TRANSPARENT );
	...
	void OnDrawItem( DRAWITEMSTRUCT* Item )
	{
		...
		else if( hIcon )
		{
			...
			DrawIcon( Item->hDC, R.left + bPressed, R.top + (R.bottom-R.top)/2-II.yHotspot + bPressed, hIcon );
...
// A coolbar-style button.
class WINDOW_API WBitmapButton : public WCoolButton
{
	W_DECLARE_CLASS(WBitmapButton,WCoolButton,CLASS_Transient);
	DECLARE_WINDOWSUBCLASS(WBitmapButton,WCoolButton,Window)
	CBitmap* pBitmap;

	// Constructor.
	WBitmapButton()
	{}
	WBitmapButton( WWindow* InOwner, INT InId=0, FDelegate InClicked=FDelegate(), DWORD InFlags=CBFF_ShowOver|CBFF_DimAway, DWORD InColor=0 )
	: WCoolButton( InOwner, InId, InClicked, InFlags, InColor )
	, pBitmap(0)
	{}
	...
	void SetBitmap( CBitmap* InBitmap )
	{
		guard(WBitmapButton::SetText);
		pBitmap = InBitmap;
		unguard;
	}
	...
	void OnDrawItem( DRAWITEMSTRUCT* Item )
	{		
		BOOL bDisabled = !IsWindowEnabled(hWnd);
		
		if( pBitmap ) // 24-bit bitmap support
		{
			UBOOL bPressed = (Item->itemState&ODS_SELECTED)!=0 || bChecked;
			RECT  R		   = Item->rcItem;
			int Adjust = (bPressed) ? 2 : 0;
			pBitmap->Draw( Item->hDC, 
				(R.right  / 2) - (pBitmap->GetWidth()  / 2) + Adjust, 
				(R.bottom / 2) - (pBitmap->GetHeight() / 2) + Adjust );
			
			DrawEdges( Item );
			DrawItemText(  Item );
		}
		else if( hbm ) // 8-bit bitmap
		{
			UBOOL	bPressed  = (Item->itemState&ODS_SELECTED)!=0 || bChecked;
			RECT	R		  = Item->rcItem;		
...
//			DrawEdges( Item );
		...
		else
		{
			if( bDisabled )
			{
				WButton::OnDrawItem( Item );
				DrawEdges( Item );
				DrawItemText(  Item );
			}
			else
			{
				// Fill the button blue
				HBRUSH hBrush = CreateSolidBrush(COLORREF(RGB(32,64,175))); // HP2 blue
...
class WINDOW_API WDialog : public WWindow
{
	...
	void OpenChildWindow( INT InControlId, UBOOL Visible )
	{
		...
		HWND hWndCreated = TCHAR_CALL_OS(
			CreateDialogParamW(hInstanceWindow/*!!*/,MAKEINTRESOURCEW(ControlId),hWndParent, (INT(APIENTRY*)(HWND,UINT,WPARAM,LPARAM))StaticWndProc,(LPARAM)this),
			CreateDialogParamA(hInstanceWindow/*!!*/,MAKEINTRESOURCEA(ControlId),hWndParent, (INT(APIENTRY*)(HWND,UINT,WPARAM,LPARAM))StaticWndProc,(LPARAM)this));
		check(hWndCreated);
		check(hWndCreated==hWnd);
		Show( Visible );
		unguard;
	...
	virtual void OnInitDialog()
	{
		...
		for( INT i=0; i<Controls.Num(); i++ )
		{
			...
			{	TCHAR buffer[512];
				appSprintf(buffer, TEXT("InitDialogName:%s\nInitDialogID:%d\nProblem DlgItem ControlID:%d\n ") , 
					*PersistentName, ControlId, Control->ControlId);
				::MessageBox( hWnd, buffer, TEXT("Error trying to GetDlgItem()!"), MB_OK );
			}
			// ---
...
class WINDOW_API WCheckListBox : public WListBox
{
	...
	/*	
		hbmOff = (HBITMAP)LoadImageA( hInstanceWindow, MAKEINTRESOURCEA(IDBM_CHECKBOX_OFF), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS );
		

		check(hbmOff);
		hbmOn = (HBITMAP)LoadImageA( hInstanceWindow, MAKEINTRESOURCEA(IDBM_CHECKBOX_ON), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS );
		check(hbmOn);
...
public:
	...
	// added gk 3/18 milestone 2

	void OnChooseScript()
	{
		extern BOOL CALLBACK editDlg(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam);
		int error;
		TCHAR Str[4096];
		HWND hwnd;
		...
		error=DialogBoxParam(hInstance,MAKEINTRESOURCE(IDD_TEXTEDIT),GetForegroundWindow(),editDlg,(LPARAM)Str);

		Property->ImportText( Str, ReadValue,PPF_Localized );
	...
	void OnItemSetFocus()
	{
		...
		&&	!(Property->PropertyFlags & CPF_EditConst) )
		{
			...
			if(Property->IsA(UStrProperty   ::StaticClass()))
			{
				...
				AddButton(TEXT("Script"),FDelegate(this,(TDelegate)OnChooseScript) );
	...
	void OnArrayInsert()
	{
		...
		if( Addr )
		{
			((TArray<BYTE>*)Addr)->InsertZeroed( Offset, Array->ElementSize );
			Parent->Collapse();
			Parent->Expand();
		}
		unguard;
	...
	void OnArrayDelete()
	{
		...
		if( Addr )
		{
			((TArray<BYTE>*)Addr)->Remove( Offset, Array->ElementSize );
			Parent->Collapse();
			Parent->Expand();
		}
		unguard;
...
class WINDOW_API WWizardPage : public WDialog
{
	...
	virtual const TCHAR* GetCancelText()
	{
		...
		if(lang==TEXT("JAP"))
		{
			return LocalizeGeneral(TEXT("CancelText"),TEXT("Startup"));
		//	return LocalizeGeneral(TEXT("FirstTime"),TEXT("Startup"));
		}
		return TEXT("_Cancel_"); //Localize(TEXT("TEXT"),TEXT("main_menu_06"),TEXT("HGame"),NULL,1);
	}
...
// A FrontEnd page.
class WINDOW_API WFrontEndPage : public WDialog
{
	W_DECLARE_ABSTRACT_CLASS(WFrontEndPage,WDialog,CLASS_Transient);
	DECLARE_WINDOWCLASS(WFrontEndPage,WDialog,Window)
	
	// Variables.
	WFrontEndDialog*	Owner;
	HBITMAP				hbmpBackground;
	
	// Constructor.
	WFrontEndPage()
	{}
	WFrontEndPage	( const TCHAR* PageName, INT ControlId, WFrontEndDialog* InOwner )
	: WDialog		( PageName, ControlId, (WWindow*)InOwner )
	, hbmpBackground( NULL )
	, Owner			( InOwner )
	{}
	...
	// WFrontEndPage interface.
	virtual void OnCurrent()
	{}
	virtual void OnPaint()
	{
		guard(WFrontEnd::OnPaint);
		PAINTSTRUCT PS;
		HDC hDC = BeginPaint( *this, &PS );
		...
		// Draw our background bitmap (if we have one)
		if( hbmpBackground )
		{			
			HDC     hdcMem = CreateCompatibleDC(hDC);
			HBITMAP hbmOld = (HBITMAP)SelectObject(hdcMem, hbmpBackground );
			RECT	BmpPos = rc;
			
			BitBlt(hDC,	(rc.right - BmpPos.right) / 2, (rc.bottom - BmpPos.bottom) / 2,
						BmpPos.right, BmpPos.bottom,
						hdcMem,
						BmpPos.left, BmpPos.top,
						SRCCOPY);
	...
	virtual const TCHAR* GetOkText()
	{
		return LocalizeGeneral("OkButton",TEXT("Window"));
	}
	...
	virtual const TCHAR* GetCancelText()
	{
		...
		return TEXT("&Cancel"); //Localize(TEXT("TEXT"),TEXT("main_menu_06"),TEXT("HGame"),NULL,1);
	}
...
// The FrontEnd frame dialog.
class WINDOW_API WFrontEndDialog : public WDialog
{
	W_DECLARE_CLASS(WFrontEndDialog,WDialog,CLASS_Transient);
	DECLARE_WINDOWCLASS(WFrontEndDialog,WDialog,Window)
	
	// Variables.
	TArray<WFrontEndPage*>	Pages;				//
	WFrontEndPage*			CurrentPage;		//
	WLabel					PageHolder;			//

	bool					bDrawBackground;
	DWORD					BackgroundColor;	// Background color
	FWindowsBitmap			BackgroundBitmap;	// Background for all FrontEnd Pages to use
	WBitmapButton			BackButton, OkButton;
	
	// Constructor.
	WFrontEndDialog		( )
	: WDialog			( TEXT("FrontEndDialog"), IDDIALOG_FrontEndDialog )
	, PageHolder		( this, IDC_PageHolder )
	, Pages				( )
	, BackButton		( this, IDC_Back, FDelegate( this, (TDelegate)OnBack ), CBFF_ShowOver|CBFF_DimAway, 0xFFFFFF )
	, OkButton			( this, IDC_OK,   FDelegate( this, (TDelegate)OnOk ),   CBFF_ShowOver|CBFF_DimAway, 0xFFFFFF )
	, bDrawBackground	( true )
	, BackgroundColor	( 0 )
	, CurrentPage		( NULL )
	{}

	void SetBackgroundColor( DWORD dwColor, bool bDraw = true )
	{
		bDrawBackground = bDraw;
		BackgroundColor = dwColor;
	...
	void OnInitDialog()
	{
		guard(WFrontEndDialog::OnInitDialog);
		WDialog::OnInitDialog();
		SendMessageX( *this, WM_SETICON, ICON_BIG, (WPARAM)LoadIconIdX(hInstance,(GIsEditor?IDICON_Editor:IDICON_Mainframe)) );
		
		// HP2 background bitmap could not have color == (9, 3, 11)
		SetBackgroundColor( RGB(9, 3, 11) );
		InvalidateRect( hWnd, NULL, false );
	...
	// WFrontEndDialog interface.
	virtual void Advance( WFrontEndPage* NewPage )
	{
		guard(WFrontEndDialog::Advanced);
		check(NewPage);
...
//			OnNext();
	...
	virtual void RefreshPage()
	{
		guard(WFrontEndDialog::RefreshPage);
		if( Pages.Num() )
		{
			WFrontEndPage* Page = Pages.Last();
			if( !Page->hWnd )
				Page->OpenChildWindow( IDC_PageHolder, 1 );
			
			BackButton  .SetVisibleText( Pages.Num()>1 ? Page->GetBackText() : NULL );
			OkButton    .SetVisibleText( Page->GetOkText() );
	...
	virtual void OnDestroy()
	{
		guard(WFrontEndDialog::OnDestroy);
		for( INT i=0; i<Pages.Num(); i++ )
			delete Pages(i);
		WDialog::OnDestroy();
		unguard;
	...
	virtual void OnOk()
	{
		guard(WFrontEndDialog::OnCancel);
		Pages.Last()->OnOk();
