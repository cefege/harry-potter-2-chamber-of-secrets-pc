...
// |  ____|  ____|      | \ | |              / ____|                    |  \/  |                                      
// | |__  | |__         |  \| | _____      __ |  __  __ _ _ __ ___   ___| \  / | ___ _ __  _   _      ___ _ __  _ __  
// |  __| |  __|        | . ` |/ _ \ \ /\ / / | |_ |/ _` | '_ ` _ \ / _ \ |\/| |/ _ \ '_ \| | | |    / __| '_ \| '_ \ 
// | |    | |____       | |\  |  __/\ V  V /| |__| | (_| | | | | | |  __/ |  | |  __/ | | | |_| | _ | (__| |_) | |_) |
// |_|    |______|      |_| \_|\___| \_/\_/  \_____|\__,_|_| |_| |_|\___|_|  |_|\___|_| |_|\__,_|(_) \___| .__/| .__/ 
//                ______                                                                                 | |   | |    
//               |______|                                                                                |_|   |_|    
//
// -----------------------------------------------------------------------------
...
HRESULT CFrontEnd::NewGameMenu_OnOpen( const HWND hWndDlg )
{
...
//	if( !m_bmpBackground.LoadBmpFile( BITMAP_FILENAME_NEWGAME_BACKGROUND ) )
//		return E_FAIL;
	
	// Setup the Title of the save slots
	SetWindowText( hWndDlg, ("FrontEnd: New Game Menu") );
	SendMessage( GetDlgItem(hWndDlg,IDC_LABEL_ACTION),		WM_SETTEXT, 0, (LPARAM)("New Game Instructions") );
...
void CFrontEnd::NewGameMenu_OnPaint( const HWND hWndDlg )
{
	RECT rc;
...
void CFrontEnd::NewGameMenu_OnButtonSlot( u32 iSlot )
{
	if( iSlot > NUM_SAVE_SLOTS ) { ErrorMessage("Invalid Slot Number: %d !", iSlot ); return; }
	
	// If this slot is NOT empty, we need to empty it
	if( !m_SaveSlots[iSlot].m_bEmpty )
	{
		// Ask the user if he is sure he wants to use this slot!
		char question[256];
		sprintf(question, "Are you sure you want to overwrite game %d?", iSlot );
		
		if( MessageBox( m_hWnd, question, "Confirm Override", MB_YESNO ) )
			return;
	}
	
	// Clean out all the old files for this slot, we are starting a new game
	EmptySaveSlotFiles( iSlot );
			
	// Launch our game using the specified slot!
	LaunchNewGame( iSlot );	
}

void CFrontEnd::NewGameMenu_OnDrawItemButtonSlot( HWND hWndDlg, LPDRAWITEMSTRUCT pDIS )
{
	assert( hWndDlg && pDIS );
...
INT_PTR CALLBACK NewGameMenu_DialogProc( HWND hWndDlg, UINT iMsg, WPARAM wParam, LPARAM lParam )
{
	switch( iMsg )
	{
		...
		case WM_MM_OPEN:
			g_FrontEnd.NewGameMenu_OnOpen( hWndDlg );
			return 0;
		...
		case WM_PAINT:
			g_FrontEnd.NewGameMenu_OnPaint( hWndDlg );
			break;
		...
		case WM_DRAWITEM:
			...
			{
				g_FrontEnd.NewGameMenu_OnDrawItemButtonSlot( hWndDlg, (LPDRAWITEMSTRUCT)lParam );
			}
		...
		case WM_COMMAND:
			...
			{
				case IDC_BUTTON_SLOT1:	g_FrontEnd.NewGameMenu_OnButtonSlot( 1 );	break;
				case IDC_BUTTON_SLOT2:	g_FrontEnd.NewGameMenu_OnButtonSlot( 2 );	break;
				case IDC_BUTTON_SLOT3:	g_FrontEnd.NewGameMenu_OnButtonSlot( 3 );	break;
				case IDC_BUTTON_SLOT4:	g_FrontEnd.NewGameMenu_OnButtonSlot( 4 );	break;
				case IDC_BUTTON_SLOT5:	g_FrontEnd.NewGameMenu_OnButtonSlot( 5 );	break;
				case IDC_BUTTON_SLOT6:	g_FrontEnd.NewGameMenu_OnButtonSlot( 6 );	break;
