...
public:
	...
	virtual void LightAndFog( FTransSample& Point, DWORD ExtraFlags )=0;
...
public:
	...
	// Inlines.
	FLOAT RandomBase( UPTRINT i ) {return RandomBases[i & RAND_MASK]; }
...
class RENDER_API URender : public URenderBase
{
	...
	UBOOL BoundVisible( FSceneNode* Frame, const FCoords& BoxCoords, FSpanBuffer* SpanBuffer, FScreenBounds& Results );
	...
	void DrawCylinder( FSceneNode* Frame, FPlane Color, DWORD LineFlags, const FCoords& Place, FLOAT Radius, FLOAT Height, FLOAT Width = -1 );
	...
	void DrawMesh( FSceneNode* Frame, FDynamicSprite* Sprite, AActor* Owner, const FCoords& Coords, DWORD PolyFlags );
	void DrawLodMesh( FSceneNode* Frame, FDynamicSprite* Sprite, AActor* Owner, const FCoords& Coords, DWORD PolyFlags );
