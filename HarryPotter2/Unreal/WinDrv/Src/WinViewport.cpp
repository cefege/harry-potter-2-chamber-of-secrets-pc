...
void UWindowsViewport::OpenWindow( DWORD InParentWindow, UBOOL IsTemporary, INT NewX, INT NewY, INT OpenX, INT OpenY )
{
	...
	bool bFullscreen = !ParseParam(appCmdLine(), TEXT("WINDOW"));
	...
	else
	{
		...
		if( OpenX==-1 )
			OpenX = bFullscreen ? CW_USEDEFAULT : 0;
		if( OpenY==-1 )
			OpenX = bFullscreen ? CW_USEDEFAULT : 0;
	...
	if( !RenDev && !GIsEditor && !ParseParam(appCmdLine(),TEXT("nohard")) )
		TryRenderDevice( TEXT("ini:Engine.Engine.GameRenderDevice"), NewX, NewY, INDEX_NONE, C->StartupFullscreen && bFullscreen );
	if( !RenDev && !GIsEditor && C->StartupFullscreen && bFullscreen )
...
//	static	DWORD	dwPreviousButtonState[2] = {0, 0};
//	DWORD			dwButtonState[3];
...
//			dwButtonState[1] = dwPreviousButtonState[0];
//			dwButtonState[2] = dwPreviousButtonState[1];
//			dwButtonState[0] = JoyInfo.dwButtons;

//			for( Index=0; Index<16; Index++,dwButtonState[0] /= 2, dwButtonState[1] /= 2, dwButtonState[2] /= 2)
			for( Index=0; Index<16; Index++, JoyInfo.dwButtons /= 2)
			{
//			   if( !Input->KeyDown(IK_Joy1+Index) && (dwButtonState[0] & 1) )
...
//			   else if( Input->KeyDown(IK_Joy1+Index) && !(dwButtonState[0] & 1)
//						&& !(dwButtonState[1] & 1) && !(dwButtonState[2] & 1))
			   {
...
//			dwPreviousButtonState[1] = dwPreviousButtonState[0];
//			dwPreviousButtonState[0] = JoyInfo.dwButtons;
...
static HFONT CreateNativeFont( const TCHAR* FontName, int Height )
{
	...
	if(lang.InStr(TEXT("JAP"))>-1)
		charSet=SHIFTJIS_CHARSET;
	else if(lang.InStr(TEXT("KOR"))>-1)
		charSet=JOHAB_CHARSET;
	else if(lang.InStr(TEXT("THA"))>-1)
		charSet=THAI_CHARSET;
	else if(lang.InStr(TEXT("CHI"))>-1)
		charSet=CHINESEBIG5_CHARSET;
	else if(lang.InStr(TEXT("TRA"))>-1)
		charSet=CHINESEBIG5_CHARSET;
	else if(lang.InStr(TEXT("SIM"))>-1)
...
//		charSet=DEFAULT_CHARSET;
		return CreateFontW( -Height, 0, 0, 0, FW_NORMAL, 0, 0, 0, charSet, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, /*AntiAlias ? ANTIALIASED_QUALITY :*/ NONANTIALIASED_QUALITY, DEFAULT_PITCH, appToUnicode(FontName) );
		}
	else
#endif
		return CreateFontA( -Height, 0, 0, 0, FW_NORMAL, 0, 0, 0, charSet, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, /*AntiAlias ? ANTIALIASED_QUALITY :*/ NONANTIALIASED_QUALITY, DEFAULT_PITCH, appToAnsi(FontName) );
}

void UWindowsViewport::DrawString( DWORD Flags, UFont* Font, INT& DrawX, INT& DrawY, const TCHAR* Text, const FPlane& Color )
{
	guard(UWindowsViewport::DrawString);

	// Create and cache the font.
	if( !Font->NativeFont )
	{
		Font->NativeFont = ::CreateNativeFont( *Font->FontName, Font->FontHeight );
		if( !Font->NativeFont )
			return;
	}
	HFONT hFont = (HFONT)Font->NativeFont;

	// Acquire a DC on the relevant draw surface.
	HDC hDC = NULL;
	if( BlitFlags & BLIT_DirectDraw )
	{
		if( !ddBackBuffer )
			return;
		ddBackBuffer->Unlock( ddSurfaceDesc.lpSurface );
		if( ddBackBuffer->GetDC( &hDC )!=0 )
			return;
	}
	else if( BlitFlags & BLIT_DibSection )
	{
		UWindowsClient* Client = GetOuterUWindowsClient();
		if( !Client || !Client->hMemScreenDC || !hBitmap ||
			!SelectObject( Client->hMemScreenDC, hBitmap ) )
			return;
		hDC = Client->hMemScreenDC;
	}
	else
	{
		hDC = (HDC)RenDev->GetOsSurface();
		if( !hDC )
			return;
	}
	...
	RECT R;
	R.left = 0;  R.right = Canvas->ClipX;
	R.top = 0;   R.bottom = Canvas->ClipY;
	INT WinFlags = DT_WORDBREAK;

	// Draw once to compute size.
//	const char* TextA = "";
#if UNICODE
	const UNICHAR* TextW = L"";
	if( GUnicodeOS )
		DrawTextW( hDC, TextW = appToUnicode(Text), -1, &R, WinFlags | DT_CALCRECT );
	else
#endif	
		DrawTextA( hDC, TCHAR_TO_ANSI(Text), -1, &R, WinFlags | DT_CALCRECT );
	...
	{
		R.left = DrawX-R.right + Canvas->OrgX;		R.right = DrawX + Canvas->OrgX;
		R.top = DrawY-R.bottom + Canvas->OrgY;		R.bottom = DrawY + Canvas->OrgY;
...
#if UNICODE
		if( GUnicodeOS )
			DrawTextW( hDC, TextW, -1, &R, WinFlags );
		else
#endif
			DrawTextA( hDC, TCHAR_TO_ANSI(Text), -1, &R, WinFlags );

	}

	if( BlitFlags & BLIT_DirectDraw )
	{
		ddBackBuffer->ReleaseDC( hDC );

		appMemzero( &ddSurfaceDesc, sizeof(ddSurfaceDesc) );
  		ddSurfaceDesc.dwSize = sizeof(ddSurfaceDesc);
		HRESULT Result = ddBackBuffer->Lock( NULL, &ddSurfaceDesc, DDLOCK_WAIT|DD_OTHERLOCKFLAGS, NULL );
  		if( Result != DD_OK )
			debugf( NAME_Warning, TEXT("DirectDraw Lock failed: %X: %s"), Result, ddError(Result) );
	...
	else if( BlitFlags & BLIT_DibSection )
	{
		...
		RenDev->GetOsSurface( hDC );

	unguard;
}

UFont* UWindowsViewport::CreateNativeFont( const TCHAR* FontName, int Height )
{
	// Create and cache the font.
	UFont* Font = new UFont;
	Font->FontName = FontName;
	Font->FontHeight = Height;
	Font->NativeFont = ::CreateNativeFont( FontName, Height );
	return Font;
}
...
UBOOL UWindowsViewport::LoadSmackerMovie(const TCHAR* lpsz_MoviePath)
{
		return false;
}

UBOOL UWindowsViewport::ShutdownSmacker( )
{
	return true;
}

UBOOL UWindowsViewport::BlitNextSmackerFrame( )	// Returns 0 on failure or when the movie ends
{
	return false;
}
...
//	OutputDebugString(TEXT("Here!!!\n"));
	...
	static UViewport* SavedMinViewport = NULL;
	...
	switch( iMessage )
	{
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_CREATE\n"));
#endif
			guard(WM_CREATE);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_HOTKEY\n"));
#endif
			return 0;
		}
		case WM_DESTROY:
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_DESTROY\n"));
#endif
			guard(WM_DESTROY);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_PAINT\n"));
#endif
			guard(WM_PAINT);
#if 01
			if (!GIsEditor && !bActive) 
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("No painting here!\n"));
#endif
				return 0;
			}
			else
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("We're bActive!\n"));
#endif
			}
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_COMMAND\n"));
#endif
			guard(WM_COMMAND);
      		switch( wParam )
			{
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_KEYDOWN/WM_SYSKEYDOWN\n"));
#endif
			guard(WM_KEYDOWN);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_KEYUP/WM_SYSKEYUP\n"));
#endif
			guard(WM_KEYUP);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_CHAR/WM_SYSCHAR\n"));
#endif
			guard(WM_CHAR);
			EInputKey Key = (EInputKey)wParam;
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_ERASEBKGND\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_SETCURSOR\n"));
#endif
			guard(WM_SETCURSOR);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_LBUTTONDBLCLK\n"));
#endif
		...
		case WM_MBUTTONDOWN:
		{
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_LBUTTONDOWN\n"));
#endif
				if( GIsEditor )
				{
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_RBUTTONDOWN\n"));
#endif
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_MBUTTONDOWN\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_MOUSEACTIVATE\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_ACTIVATE\n"));
#endif

			if(GDebugger)			
				return 0;		//gk debugger need to keep the game running in the background

			//OutputDebugString(TEXT("In WinViewport\n"));
			/*BOOL*/ bActive = ((LOWORD(wParam) == WA_ACTIVE)  ||  (LOWORD(wParam) == WA_CLICKACTIVE));
			//OutputDebugString( bActive ? TEXT("Activating...\n"):TEXT("Deactvating...\n"));
			...
			if( wParam==WA_INACTIVE)
			{
				...
				{
					SavedMinViewport = Client->Engine->Audio->GetViewport();
			...
			else
			{
				if(SavedMinViewport)
				{
					Client->Engine->Audio->SetViewport( SavedMinViewport );
					SavedMinViewport=NULL;
				}
			}
			if (!GIsEditor && (BlitFlags & BLIT_Fullscreen))
			{
				if (bActive)
		...
		case WM_MBUTTONUP:
		{
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_LBUTTONUP\n"));
#endif
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_MBUTTONUP\n"));
#endif
			...
			{
#if MSG_OUTPUT
				OutputDebugString(TEXT("MESSAGE:  WM_RBUTTONUP\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_ENTERMENULOOP\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_EXITMENULOOP\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_CANCELMODE\n"));
#endif
			guard(WM_CANCELMODE);
			SetDrag( 0 );
			return 0;
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_MOUSEWHEEL\n"));
#endif
			guard(WM_MOUSEWHEEL);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_MOUSEMOVE\n"));
#endif
			guard(WM_MOUSEMOVE);
			if( !diMouse || !AcquiredDIMouse )
			{
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_SIZE\n"));
#endif
			guard(WM_SIZE);
			INT NewX = LOWORD(lParam);
			INT NewY = HIWORD(lParam);

			if ((!GIsEditor)  &&  (wParam == SIZE_MINIMIZED)  &&  ((NewX == 0)  &&  (NewY == 0)))
			{
				OutputDebugString(TEXT("Forced minimization\n"));
				ShowWindow(Window->hWnd, SW_MINIMIZE);
				if(Client->Engine->Audio)
				{
					SavedMinViewport = Client->Engine->Audio->GetViewport();
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_KILLFOCUS\n"));
#endif
			guard(WM_KILLFOCUS);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_SETFOCUS\n"));
#endif
			guard(WM_SETFOCUS);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_SYSCOMMAND\n"));
#endif
			guard(WM_SYSCOMMAND);
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_POWER\n"));
#endif
			guard(WM_POWER);
			if( wParam )
			{
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_DISPLAYCHANGE\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_WININICHANGE\n"));
#endif
		...
		{
#if MSG_OUTPUT
			OutputDebugString(TEXT("MESSAGE:  WM_UNKNOWN\n"));
#endif
			guard(WM_UNKNOWN);
			return DefWindowProcX( Window->hWnd, iMessage, wParam, lParam );
			unguard;
...
//	OutputDebugString(TEXT("In ToggleFullscreen()\n"));
	if( BlitFlags & BLIT_Fullscreen )
	{
		EndFullscreen();
	}
