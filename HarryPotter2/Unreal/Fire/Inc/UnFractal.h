...
class DLL_EXPORT_CLASS UFireTexture : public UFractalTexture
{
	...
	BYTE		NativeBasePadding[6]; // Preserve package-79's completed UFractalTexture base size.
	// Persistent variables.
    BYTE		SparkType;    
    BYTE        RenderHeat;
	BYTE		NativeBoolPadding[2]; // Start bRising in its own package-aligned BITFIELD word.
	BITFIELD	bRising:1;
	...
	// Sparks
	INT         ActiveSparkNum;
	TArray<FSpark> Sparks GCC_PACK(4);
...
class DLL_EXPORT_CLASS UWaterTexture : public UFractalTexture
{
	...
	BYTE		NativeFractalBasePadding[6]; // Preserve package-79's completed UFractalTexture base size.
    ...
    // Non-Persistent variables mirrored by the package-79 class layout:
    INT     SourceFields; // 32-bit reflected placeholder; host allocation is owner-keyed.
    BYTE    RenderTable[1028];
    BYTE	WaveTable[1536];
    BYTE	WaveParity;
	INT     OldWaveAmp;
...
class DLL_EXPORT_CLASS UWetTexture : public UWaterTexture
{
	...
	INT         LocalSourceBitmap; // 32-bit reflected placeholder; host allocation is owner-keyed.
...
class DLL_EXPORT_CLASS UIceTexture : public UFractalTexture
{
	...
	// Persistent variables.
    UTexture*   GlassTexture GCC_PACK(4);
    UTexture*   SourceTexture GCC_PACK(4);
    BYTE        PanningStyle;
	BYTE        TimeMethod;
    BYTE        HorizPanSpeed;
    BYTE        VertPanSpeed;
	BYTE        Frequency;
	BYTE        Amplitude;
	BYTE		NativeBoolPadding[2]; // Start MoveIce in its own package-aligned BITFIELD word.
	...
	INT         OldUDisp;
	INT         OldVDisp;
	UTexture*   OldGlassTex GCC_PACK(4);
	UTexture*	OldSourceTex GCC_PACK(4);
	INT         LocalSource;
	INT         ForceRefresh;
