//===============================================================================
//  [skWoodFirewoodContainer] 
//===============================================================================

class skWoodFirewoodContainer extends HPMeshActor;
#exec MESH  MODELIMPORT MESH=skWoodFirewoodContainerMesh MODELFILE=models\skWoodFirewoodContainer.PSK LODSTYLE=10
#exec MESH  ORIGIN MESH=skWoodFirewoodContainerMesh X=0 Y=0 Z=0 YAW=0 PITCH=0 ROLL=0
#exec ANIM  IMPORT ANIM=skWoodFirewoodContainerAnims ANIMFILE=models\skWoodFirewoodContainer.PSA COMPRESS=1 MAXKEYS=999999 IMPORTSEQS=1
#exec MESHMAP   SCALE MESHMAP=skWoodFirewoodContainerMesh X=1.0 Y=1.0 Z=1.0
#exec MESH  DEFAULTANIM MESH=skWoodFirewoodContainerMesh ANIM=skWoodFirewoodContainerAnims

// Digest and compress the animation data. Must come after the sequence declarations.
// 'VERBOSE' gives more debugging info in UCC.log 
#exec ANIM DIGEST  ANIM=skWoodFirewoodContainerAnims VERBOSE

#EXEC TEXTURE IMPORT NAME=skWoodFirewoodContainerTex0  FILE=TEXTURES\firewood_128.bmp  GROUP=Skins

#EXEC MESHMAP SETTEXTURE MESHMAP=skWoodFirewoodContainerMesh NUM=0 TEXTURE=skWoodFirewoodContainerTex0

// Original material [0] is [SKIN00.TWOSIDED] SkinIndex: 0 Bitmap: firewood_128.bmp  Path: C:\Harry Potter\ART\Objects\Logs_Firewood_Wood\Wood in Container 


defaultproperties
{
    Mesh=skWoodFirewoodContainerMesh
    DrawType=DT_Mesh
    bStatic=False
}

