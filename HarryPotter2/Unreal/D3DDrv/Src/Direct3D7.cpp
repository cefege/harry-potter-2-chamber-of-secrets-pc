...
#define ACTORPOLY_VERTEXBUFFER_SIZE    1024
#define ACTORPOLY_VERTEX_MAX		    256		// Optimum size of a single primitive.
	...
	struct FTexFiller1555_P8 : public FTexFiller
	{
		...
		void BeginUpload( UD3DRenderDevice* Device, FTexInfo* ti, const FTextureInfo& Info, DWORD PolyFlags )
		{
			...
			(
				...
				appRound(1.0f*0x07fffffff/Max<INT>(ti->MaxColor.R,1)), 0x7C000000,
				appRound(1.0f*0x003ffffff/Max<INT>(ti->MaxColor.G,1)), 0x03E00000,
				appRound(1.0f*0x0001fffff/Max<INT>(ti->MaxColor.B,1)), 0x001F0000
				// gk 11/22 switched to 1.0 to please the artists
...
#if 0
		if (Direct3DDevice7 == NULL)
		{
			OutputDebugString(TEXT("D3D Device has been trashed\n"));
		}
		else
		{
			if (BackSurface7 == NULL)
			{
				OutputDebugString(TEXT("Back Buffer has been trashed\n"));
			}
			else
			{
				if (PrimarySurface7 == NULL)
				{
					OutputDebugString(TEXT("Primary Buffer has been trashed\n"));
				}
				else
				{
					OutputDebugString(TEXT("A-OK!\n"));
				}
			}
		}

//		HRESULT hRes = Direct3DDevice7->Present(NULL, NULL, NULL, NULL);
//		while (hRes == D3DERR_DEVICELOST)
//		{
//			while (hRes != D3DERR_DEVICENOTRESET)
//			{
//				Sleep(1000);
//				hRes = DirectDraw7->TestCooperativeLevel();
//			}
			//hRes = DirectDraw7->Reset();
//		}
#endif
...
//*********************************************************************************************************
	...
	void DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet, DWORD PolyFlags, BYTE cAlpha )
	{
		...
		UBOOL IsSelected = GIsEditor && ( PolyFlags & PF_Selected )!= 0;
		...
		if( UseMultitexture && Surface.LightMap!=NULL && Surface.MacroTexture==NULL )
		{
			...
			SetTexture( 0, *Surface.Texture, PolyFlags, 0 );
			SetTexture( 1, *Surface.LightMap, 0, 0 );
			// PF_Memorize to signify multitexturing.
			SetBlending( PolyFlags|PF_Memorized );

			// Set up all poly vertices.
			FUnrealVertex* V=VertsUWindow.Lock(VertexCount);
			
			D3DCOLOR clr	= (FColor(Stages[0].MaxColor * Stages[1].MaxColor).TrueColor()) | (((DWORD)cAlpha) << 24);
		...
		if( IsSelected )
		{
			...
			Direct3DDevice7->SetRenderState( D3DRENDERSTATE_ZFUNC, PolyFlags & PF_Masked ? D3DCMP_LESSEQUAL : D3DCMP_EQUAL );
	...
	int MaxVertices()
	{
		return ACTORPOLY_VERTEX_MAX;
	}

	void DrawTriangles( FSceneNode* Frame, FTextureInfo& Info, FTransTexture** Pts, INT NumPts, _WORD* Indices, INT NumIndices, DWORD PolyFlags, FSpanBuffer* )
	{
		guardSlow(UD3DRenderDevice::DrawTriangles);
		...
		FLOAT ProjZ = Frame->RProj.Z;
		if( (GUglyHackFlags&1) && ViewportColorBits==16 )
			ProjZ *= 0.25f;
		FLOAT AddX = Frame->XB - 0.5f;
		FLOAT AddY = Frame->YB - 0.5f;

		FVector ColorScale = Stages[0].MaxColor*255.f;

		// Set up verts.
		checkSlow( NumPts <= ACTORPOLY_VERTEX_MAX );
		FD3DVertex* V = VertsTLWindow.Lock(NumPts);

		for( INT i=0; i<NumPts; i++, V++ )
		{
			FTransTexture* P = Pts[i];
			V->dvRHW  = P->RZ * ProjZ;
			V->dvSZ   = ProjectionMatrix._33 + ProjectionMatrix._43 * V->dvRHW;			
			V->dvSX   = P->ScreenX + AddX;
			V->dvSY   = P->ScreenY + AddY;
			...
			{					
				...
				V->dcSpecular = RGBA_MAKE( 
					appRound(P->Fog.X*255.f ), 
					appRound(P->Fog.Y*255.f ), 
					appRound(P->Fog.Z*255.f ), 
					255 );
				V->dcColor =    RGBA_MAKE( 
					appRound(P->Light.X*ColorScale.X), 
					appRound(P->Light.Y*ColorScale.Y), 
					appRound(P->Light.Z*ColorScale.Z), 
					appRound(P->Light.W*255.f) );
			}
			else
			{
				V->dcSpecular = 0;
		...
		// Allow both indexed and non-indexed drawing.
		if( Indices )
			Direct3DDevice7->DrawIndexedPrimitiveVB
			( 
				D3DPT_TRIANGLELIST, VertsTLWindow.VertexBuffer7, 
				VertsTLWindow.Unlock(), NumPts, 
				Indices, NumIndices, 0 
			);
		else
			Direct3DDevice7->DrawPrimitiveVB
			( 
				NumIndices == NumPts ? D3DPT_TRIANGLEFAN : D3DPT_TRIANGLELIST, 
				VertsTLWindow.VertexBuffer7, VertsTLWindow.Unlock(), NumPts, 0
			);
	...
	{
		guardSlow(UD3DRenderDevice::DrawTile);
	...
	void Draw2DLine( FSceneNode* Frame, FPlane Color, DWORD LineFlags, FVector P1, FVector P2 )
	{
		...
		FLOAT D1   = LineFlags & LINE_DepthCued ? Clamp(1.f-.001f/P1.Z, 0.2f, 1.f) : 1.f;
		V->dcColor = FColor(Color * D1).TrueColor() | 0xff000000;
		V++;
		V->dvSX    = P2.X - 0.5f;
		V->dvSY    = P2.Y - 0.5f;
		V->dvRHW   = 1.f; 
		V->dvSZ    = ProjectionMatrix._33 + ProjectionMatrix._43 * 1.f;
		FLOAT D2   = LineFlags & LINE_DepthCued ? Clamp(1.f-.001f/P2.Z, 0.2f, 1.f) : 1.f;
		V->dcColor = FColor(Color * D2).TrueColor() | 0xff000000;
	...
	void UD3DRenderDevice::PopHit( INT Count, UBOOL bForce )
	{
		...
		if( Hit || bForce )
		{
			if( HitSize && HitStack.Num() <= *HitSize )
			{
	...
	void* GetOsSurface( void* Release, bool bFrontBuffer ) 
	{
		guard(UD3DRenderDevice::GetOsSurface);
		if( !Release )
		{
			HDC hDC = NULL;
			if(bFrontBuffer)
				PrimarySurface7->GetDC( &hDC );
			else
			{
				Direct3DDevice7->EndScene();
				BackSurface7->GetDC( &hDC );
			}
			return hDC;
		}
		else
		{
			if(bFrontBuffer)
				PrimarySurface7->ReleaseDC( (HDC)Release );
			else
			{
				BackSurface7->ReleaseDC( (HDC)Release );
	...
	DWORD AdjustPolyFlags( DWORD PolyFlags )
	{
	...
	{
		PolyFlags = AdjustPolyFlags(PolyFlags);
		...
		if( Xor & (PF_Translucent|PF_Modulated|PF_Invisible|PF_Occlude|PF_Masked|PF_Highlighted|PF_NoSmooth|PF_RenderFog|PF_Memorized) )
		{
			...
			if( Xor&(PF_Invisible|PF_Translucent|PF_Modulated|PF_Highlighted) )
			{
				...
				// Check for Lumos Affected polyFlags
				else if( PolyFlags & PF_LumosAffected )
				{
					Direct3DDevice7->SetRenderState( D3DRENDERSTATE_ALPHABLENDENABLE, TRUE );
					
					// HP2 ( if PF_Translucent then this is a translucent poly that uses alpha from vertices )
					Direct3DDevice7->SetRenderState( D3DRENDERSTATE_SRCBLEND,  D3DBLEND_INVSRCALPHA );
					Direct3DDevice7->SetRenderState( D3DRENDERSTATE_DESTBLEND, D3DBLEND_SRCALPHA );
				}
				...
				else if( PolyFlags & PF_Highlighted )
				{
					...
					if( PolyFlags & PF_Translucent )
						// Post-multiplied source.
						Direct3DDevice7->SetRenderState( D3DRENDERSTATE_SRCBLEND, D3DBLEND_SRCALPHA );
					else
						Direct3DDevice7->SetRenderState( D3DRENDERSTATE_SRCBLEND, D3DBLEND_ONE );
				}
				else if( PolyFlags & PF_Translucent )
			...
			if( Xor & PF_Memorized )
			{
				...
				Direct3DDevice7->SetTextureStageState( 1, D3DTSS_COLOROP, (PolyFlags&PF_Memorized) ? D3DTOP_MODULATE2X  : D3DTOP_DISABLE );
		...
		switch( StageSetupScheme )
		{
			...
			case SSS_3dfx:
			{
				...
				if( UseMultitexture )
				{
					...
					Direct3DDevice7->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_TEXTURE );
					Direct3DDevice7->SetTextureStageState( 1, D3DTSS_COLORARG2, D3DTA_CURRENT );
					Direct3DDevice7->SetTextureStageState( 1, D3DTSS_COLOROP,   D3DTOP_MODULATE2X );
					Direct3DDevice7->SetTextureStageState( 1, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
					Direct3DDevice7->SetTextureStageState( 1, D3DTSS_ALPHAARG2, D3DTA_CURRENT );
	...
	Retry:
		...
		GConfig->SetString(TEXT("CARDCAP"),TEXT("VRAM"),*FString::Printf(TEXT("%i"),DDDriverCaps.dwVidMemTotal/1024));
		unguard;
		...
		debugf(NAME_Init,TEXT("Direct3D support - internal revision 1.9b"));
		...
		// Update the viewport.
		verify(Viewport->ResizeViewport( (ViewportFullscreen ? BLIT_Fullscreen : 0) | BLIT_Direct3D | BLIT_HardwarePaint, NewX, ViewportY, ViewportColorBits / 8 ));
