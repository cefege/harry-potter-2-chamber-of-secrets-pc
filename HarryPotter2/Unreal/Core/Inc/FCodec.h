...
public:
	UBOOL Encode( FArchive& In, FArchive& Out, FCodecNotify* Notify=NULL )
	{
		...
		while( !In.AtEnd() )
		{
			...
			appQsort( &CompressPosition(0), CompressLength+1, appCheckedIntSize(sizeof(INT)), (QSORT_COMPARE)ClampedBufferCompare );
			for( i=0; i<CompressLength+1; i++ )
...
public:
	...
	UBOOL Decode( FArchive& In, FArchive& Out, FCodecNotify* Notify=NULL )
	{
		...
		while( Remaining-- > 0 )
		{
			check(!Reader.AtEnd());
			FHuffman* Node = &Root;
			for( ; Node->Ch==-1; Node=Node->Child(Reader.ReadBit()) );
			BYTE B = Node->Ch;
			Out << B;
