...
public:
	...
	// *********************** Added for LoadPersistence support ******************************

/*	FArchive& operator<<( UObject*& Obj )
	{
		guard(FArchiveFileReader<<UObject);
		if( Obj )
		{
			Obj->Serialize( *this );
		}
...
*/
	...
	{		
		...
		{
			FName NewName;
			// load size of name string, then load the string itself
			Serialize(&iLen, sizeof(iLen) );
			Serialize(&buffer, iLen );

			// convert the string into a name
			NewName = FName(buffer);
			
			// If we have a valid name then update the current name
			if( NewName.IsValid() )
				Name = NewName;
		}
		return *this;
...
public:
	...
	// *********************** Added for SavePersistence support ******************************
/*	FArchive& operator<<( UObject*& Obj )
	{
		guardSlow(FArchiveFileWriter<<UObject);
...
*/
	...
	FArchive& operator<<( FName& Name )
	{		
		guard(FArchiveFileWriter<<Name);

		TCHAR	buffer[256];
		int		iLen;
		if( ArIsSaving )
		{
			// convert the name into a string then compute the string length
			appSprintf( buffer, TEXT("%s"), *Name );
			iLen = (appStrlen(buffer)+1)*sizeof(TCHAR);
			
			// save size of name string then name string itself
			Serialize(&iLen, sizeof(iLen) );
			Serialize(&buffer, iLen );
...
public:
	...
	UBOOL Move( const TCHAR* Dest, const TCHAR* Src, UBOOL Replace=1, UBOOL EvenIfReadOnly=0, UBOOL Attributes=0 )
	{
		...
		Result = Delete( Dest, 0, 1 );
		if( !Result )
			debugf( NAME_Warning, TEXT("Error deleting file '%s' Error %x"), Dest, Result);

		Result = TCHAR_CALL_OS( MoveFileW(Src,Dest), MoveFileA(TCHAR_TO_ANSI(Src),TCHAR_TO_ANSI(Dest)) );
		if( !Result )
			debugf( NAME_Warning, TEXT("Error moving file '%s' to '%s' Error %x"), Src, Dest, Result);
		return Result!=0;
		unguard;
	}
	...
	TArray<FString> FindFiles( const TCHAR* Filename, UBOOL Files, UBOOL Directories )
	{
		...
		FPushMemTag Push(TEXT("FindFiles"));
