...
#if __UNIX__
// Convert an Unreal logical filename to normalized UTF-8 for POSIX APIs.
CORE_API UBOOL appToUtf8NativePath( const TCHAR* Filename, std::string& Out );
#endif

// Open an Unreal logical filename through the host-native stdio boundary.
CORE_API FILE* appFopen( const TCHAR* Filename, const ANSICHAR* Mode );

// Return the component after the last native or legacy path separator.
...
/*-----------------------------------------------------------------------------
	Persistent Actor cache management functions.
-----------------------------------------------------------------------------*/
...
CORE_API const TCHAR* appFromAnsi(TArray<TCHAR> &, const ANSICHAR* Str );
...
inline DWORD appCheckedAllocSize( SIZE_T Size )
{
	if( Size > static_cast<SIZE_T>(MAXDWORD) )
		appErrorf( TEXT("Allocation size exceeds the 32-bit allocator contract") );
	return static_cast<DWORD>(Size);
}
inline INT appCheckedIntSize( SIZE_T Size )
{
	if( Size > static_cast<SIZE_T>(MAXINT) )
		appErrorf( TEXT("Size exceeds the signed 32-bit API contract") );
...
{
	guardSlow(new);
	return appMalloc( appCheckedAllocSize(Size), Tag );
	unguardSlow;
}
inline void* operator new( SIZE_T Size )
{
	guardSlow(new);
	return appMalloc( appCheckedAllocSize(Size), TEXT("new") );
	unguardSlow;
}
