...
#define ANY_PACKAGE (reinterpret_cast<UObject*>(~static_cast<UPTRINT>(0)))
...
#define VERIFY_CLASS_OFFSET(Pre,ClassName,Member) \
	{const INT CppOffset = static_cast<INT>(__builtin_offsetof(Pre##ClassName,Member)); \
	for( TFieldIterator<UProperty> It( FindObjectChecked<UClass>( Pre##ClassName::StaticClass()->GetOuter(), TEXT(#ClassName) ) ); It; ++It ) \
		if( appStricmp(It->GetName(),TEXT(#Member))==0 ) \
			if( It->Offset != CppOffset ) \
				appErrorf(TEXT("Class %s Member %s problem: Script=%i C++=%i"), TEXT(#ClassName), TEXT(#Member), It->Offset, CppOffset );}
...
// Register a class at startup time.
	...
	#define IMPLEMENT_CLASS(TClass) \
		...
		( \
			...
			appCheckedIntSize(sizeof(TClass)), \
...
// Define the package of the current DLL being compiled.
	...
	#define IMPLEMENT_PACKAGE(pkg) \
		extern "C" \
		{ \
			TCHAR LocalPackageName##pkg[] = TEXT(#pkg); \
			BYTE GLoaded##pkg; \
		}
#elif _MSC_VER
	#define IMPLEMENT_PACKAGE(pkg) \
...
public:
	...
	DECLARE_FUNCTION(execDynArrayCount)
...
template< class T > T* ConstructObject( UClass* Class, UObject* Outer=reinterpret_cast<UObject*>(~static_cast<UPTRINT>(0)), FName Name=NAME_None, DWORD SetFlags=0 );

// Construct an object of the template class type.
template< class T > T* ConstructObject( UObject* Parent=reinterpret_cast<UObject*>(~static_cast<UPTRINT>(0)), FName Name=NAME_None, DWORD SetFlags=0 )
{
	return ConstructObject<T>( T::StaticClass(), Parent, Name, SetFlags );
}
