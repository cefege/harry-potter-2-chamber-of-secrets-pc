...
class CORE_API UStruct : public UField
{
	...
	// HP2
	//
	// This function is similar to SerializeTaggedProperties but it includes special code for persistence
	void SerializeTaggedPersistentProperties( FArchive& Ar, BYTE* Data, UClass* DefaultsClass );	
...
template <class T> TFieldIterator<T>::TFieldIterator( UStruct* InStruct )
: Struct( InStruct )
...
template <class T> void TFieldIterator<T>::IterateToNext()
{
	while( Struct )
	{
		while( Field )
...
class CORE_API UClass : public UState
{
	...
	UClass( ENativeConstructor, INT InSize, DWORD InClassFlags, UClass* InBaseClass, UClass* InWithinClass, FGuid InGuid, const TCHAR* InNameStr, const TCHAR* InPackageName, const TCHAR* InClassConfigName, DWORD InFlags, void(*InClassConstructor)(void*), void(UObject::*InClassStaticConstructor)() );
	UClass( EStaticConstructor, INT InSize, DWORD InClassFlags, FGuid InGuid, const TCHAR* InNameStr, const TCHAR* InPackageName, const TCHAR* InClassConfigName, DWORD InFlags, void(*InClassConstructor)(void*), void(UObject::*InClassStaticConstructor)() );
...
template< class T > T* ConstructObject( UClass* Class, UObject* Outer, FName Name, DWORD SetFlags )
{
	check(Class->IsChildOf(T::StaticClass()));
	if( Outer==reinterpret_cast<UObject*>(~static_cast<UPTRINT>(0)) )
		Outer = static_cast<UObject*>(UObject::GetTransientPackage());
