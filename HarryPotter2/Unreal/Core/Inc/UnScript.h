...
#endif
// pre debugger code #define P_FINISH                      Stack.Code++;
//DEBUGGER
...
,	Parent		(NULL)
{}
inline FFrame::FFrame( UObject* InObject, UStruct* InNode, INT CodeOffset, void* InLocals, FFrame* InParent )
:	Node		(InNode)
,	Object		(InObject)
...
{
	INT Result;
	appMemcpy( &Result, Code, appCheckedIntSize(sizeof(Result)) );
	Code += sizeof(Result);
	return Result;
}
inline UObject* FFrame::ReadObject()
{
	UObject* Result = PeekObject();
	Code += sizeof(INT);
	return Result;
}
inline UObject* FFrame::PeekObject() const
{
	INT ObjectIndex;
	appMemcpy( &ObjectIndex, Code, appCheckedIntSize(sizeof(ObjectIndex)) );
	return UObject::GetIndexedObject( ObjectIndex );
}
...
{
	INT NameIndex;
	appMemcpy( &NameIndex, Code, appCheckedIntSize(sizeof(NameIndex)) );
	Code += sizeof(NameIndex);
	return FName( static_cast<EName>(NameIndex) );
}
