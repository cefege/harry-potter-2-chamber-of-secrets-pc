...
inline QWORD MakeCacheID( ECacheIDBase Base, const UObject* Frame )
{
	return (Base) + ((Frame?(QWORD)Frame->GetIndex():(QWORD)0) << 8);
}

inline QWORD MakeCacheID( ECacheIDBase Base, const UObject* Obj, const UObject* Frame )
{
	return (Base) + ((Obj?Obj->GetIndex():0) << 8) + ((Frame?(QWORD)Frame->GetIndex():(QWORD)0) << 32);
}

inline QWORD MakeCacheID( ECacheIDBase Base, DWORD Word, DWORD Byte, const UObject* Frame )
{
	return (Base) + (Byte<<8) + (Word<<16) + ((Frame?(QWORD)Frame->GetIndex():(QWORD)0) << 32);
}

inline QWORD MakeCacheID( ECacheIDBase Base, DWORD ByteA, DWORD ByteB, DWORD ByteC, const UObject* Frame )
{
