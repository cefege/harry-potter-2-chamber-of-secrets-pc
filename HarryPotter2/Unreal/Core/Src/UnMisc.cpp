...
static std::uint32_t appArchiveHostScalar( const TCHAR* Text, INT Length, INT& Index )
{
	std::uint32_t Scalar = static_cast<std::uint32_t>(Text[Index++]);
	if( sizeof(TCHAR)==sizeof(UNICHAR) )
	{
		if( Scalar>=0xd800u && Scalar<=0xdbffu )
		{
			if( Index<Length )
			{
				const std::uint32_t Low = static_cast<std::uint32_t>(Text[Index]);
				if( Low>=0xdc00u && Low<=0xdfffu )
					{ ++Index; return 0x10000u + ((Scalar-0xd800u)<<10) + (Low-0xdc00u); }
			}
			return 0xfffdu;
		}
	...
	else if( Scalar>0x10ffffu || (Scalar>=0xd800u && Scalar<=0xdfffu) )
		return 0xfffdu;
	return Scalar;
}
...
CORE_API FArchive& operator<<( FArchive& Ar, FString& A )
{
	...
	if( Ar.IsSaving() )
	{
		...
		{
			INT WireUnits = 0;
			for( INT Index=0; Index<A.Num(); )
			{
				const std::uint32_t Scalar = appArchiveHostScalar(*A,A.Num(),Index);
				const INT Added = Scalar>0xffffu ? 2 : 1;
				if( WireUnits>MAXINT-Added )
					appErrorf(TEXT("FString exceeds the compact-index wire contract"));
				WireUnits += Added;
			}
			SaveNum = -WireUnits;
		}
	...
	if( Ar.IsLoading() )
	{
		const INT WireUnits = Abs(SaveNum);
		A.ArrayMax = A.ArrayNum = WireUnits;
		A.Realloc( appCheckedIntSize(sizeof(TCHAR)) );
		if( SaveNum>=0 )
		{
			for( INT i=0; i<WireUnits; i++ )
				{ ANSICHAR ACh; Ar << *(BYTE*)&ACh; A(i)=FromAnsi(ACh); }
		}
		else
		{
			TArray<UNICHAR> Wire(WireUnits);
			for( INT i=0; i<WireUnits; ++i )
				Ar << Wire(i);
			INT Out = 0;
			for( INT i=0; i<WireUnits; ++i )
			{
				std::uint32_t Scalar = Wire(i);
				if( Scalar>=0xd800u && Scalar<=0xdbffu )
				{
					if( i+1<WireUnits && Wire(i+1)>=0xdc00u && Wire(i+1)<=0xdfffu )
						{ Scalar=0x10000u+((Scalar-0xd800u)<<10)+(Wire(++i)-0xdc00u); }
					else Scalar=0xfffdu;
				}
				else if( Scalar>=0xdc00u && Scalar<=0xdfffu )
					Scalar=0xfffdu;
				if( sizeof(TCHAR)==sizeof(UNICHAR) && Scalar>0xffffu )
				{
					Scalar -= 0x10000u;
					A(Out++)=static_cast<TCHAR>(0xd800u+(Scalar>>10));
					A(Out++)=static_cast<TCHAR>(0xdc00u+(Scalar&0x3ffu));
				}
				else
					A(Out++)=static_cast<TCHAR>(Scalar);
			}
			A.ArrayNum = Out;
		}
		if( A.Num()==1 && A(0)==0 )
	...
	else
	{
		for( INT Index=0; Index<A.Num(); )
		{
			std::uint32_t Scalar = appArchiveHostScalar(*A,A.Num(),Index);
			if( Scalar>0xffffu )
			{
				Scalar -= 0x10000u;
				UNICHAR High=static_cast<UNICHAR>(0xd800u+(Scalar>>10));
				UNICHAR Low =static_cast<UNICHAR>(0xdc00u+(Scalar&0x3ffu));
				Ar << High << Low;
			}
			else
			{
				UNICHAR Unit=static_cast<UNICHAR>(Scalar);
				Ar << Unit;
			}
		}
	}
...
CORE_API FArchive& operator<<( FArchive& Ar, FTime& T )
{
	...
	Ar.ByteOrderSerialize( &D.m, appCheckedIntSize(sizeof(D.m)) );
...
CORE_API TCHAR* appStaticString1024()
{
	...
	static TCHAR Results[128][1024];
	static INT Count=0;
	TCHAR* Result = Results[Count++ & 127];
	*Result = 0;
	return Result;
	unguard;
}
...
static UNICHAR* appUnicodeStaticString1024()
{
	...
	static INT Count=0;
	UNICHAR* Result = Results[Count++ & 127];
	*Result = 0;
	return Result;
}
#endif
...
CORE_API void appMD5Final ( BYTE* digest, FMD5Context* context )
{
	...
	appMemset( context, 0, appCheckedIntSize(sizeof(*context)) );
}
...
CORE_API void appMD5Transform( DWORD* state, BYTE* block )
{
	...
	// Zeroize sensitive information.
	appMemset( x, 0, appCheckedIntSize(sizeof(x)) );
}
...
static UBOOL appIsHighUtf16Surrogate( std::uint32_t CodeUnit )
{
	return CodeUnit>=0xd800u && CodeUnit<=0xdbffu;
}

static UBOOL appIsLowUtf16Surrogate( std::uint32_t CodeUnit )
{
	return CodeUnit>=0xdc00u && CodeUnit<=0xdfffu;
}

static UBOOL appReadHostCodePoint( const TCHAR* Text, INT Length, INT& Index, std::uint32_t& CodePoint )
{
	CodePoint = sizeof(TCHAR)==1
		? static_cast<std::uint32_t>(static_cast<ANSICHARU>(Text[Index++]))
		: static_cast<std::uint32_t>(Text[Index++]);
	if( sizeof(TCHAR)==sizeof(UNICHAR) )
	{
		if( appIsHighUtf16Surrogate(CodePoint) )
		{
			if( Index>=Length )
				return 0;
			const std::uint32_t Low = static_cast<std::uint32_t>( Text[Index++] );
			if( !appIsLowUtf16Surrogate(Low) )
				return 0;
			CodePoint = 0x10000u + ((CodePoint-0xd800u)<<10) + (Low-0xdc00u);
		}
		else if( appIsLowUtf16Surrogate(CodePoint) )
		{
			return 0;
		}
	}
	else if( appIsHighUtf16Surrogate(CodePoint) || appIsLowUtf16Surrogate(CodePoint) || CodePoint>0x10ffffu )
	{
		return 0;
	}
	return 1;
...
static UBOOL appEncodeUtf16LE( const FString& String, UBOOL& SaveAsUnicode, TArray<BYTE>& Bytes )
{
	const INT MaxUtf16Units = (MAXINT-2)/2;
	INT Utf16Units = 0;
	SaveAsUnicode = 0;

	for( INT Index=0; Index<String.Len(); )
	{
		std::uint32_t CodePoint;
		if( !appReadHostCodePoint(*String, String.Len(), Index, CodePoint) )
			return 0;

		const INT AddedUnits = CodePoint>0xffffu ? 2 : 1;
		if( Utf16Units>MaxUtf16Units-AddedUnits )
			return 0;
		Utf16Units += AddedUnits;
		if( CodePoint>0xffu )
			SaveAsUnicode = 1;
	}
	...
	for( INT Index=0; Index<String.Len(); )
	{
		...
		{
			CodePoint -= 0x10000u;
			const std::uint16_t High = static_cast<std::uint16_t>( 0xd800u + (CodePoint>>10) );
			const std::uint16_t Low  = static_cast<std::uint16_t>( 0xdc00u + (CodePoint&0x3ffu) );
			Bytes(Out++) = static_cast<BYTE>( High&0xffu );
			Bytes(Out++) = static_cast<BYTE>( High>>8 );
			Bytes(Out++) = static_cast<BYTE>( Low&0xffu );
			Bytes(Out++) = static_cast<BYTE>( Low>>8 );
		}
		else
		{
			const std::uint16_t Unit = static_cast<std::uint16_t>( CodePoint );
			Bytes(Out++) = static_cast<BYTE>( Unit&0xffu );
			Bytes(Out++) = static_cast<BYTE>( Unit>>8 );
		}
	}
	return 1;
}

static std::uint16_t appReadUtf16Unit( const BYTE* Bytes, INT Offset, UBOOL LittleEndian )
{
	if( LittleEndian )
		return static_cast<std::uint16_t>( Bytes[Offset] | (static_cast<std::uint16_t>(Bytes[Offset+1])<<8) );
	return static_cast<std::uint16_t>( Bytes[Offset+1] | (static_cast<std::uint16_t>(Bytes[Offset])<<8) );
}

static UBOOL appDecodeUtf16( const BYTE* Bytes, INT ByteCount, UBOOL LittleEndian, TArray<TCHAR>& Result )
{
	if( ByteCount&1 )
		return 0;
	...
	{
		const std::uint16_t First = appReadUtf16Unit( Bytes, Offset, LittleEndian );
		INT AddedChars = 1;
		if( appIsHighUtf16Surrogate(First) )
		{
			if( Offset+2>=ByteCount )
				return 0;
			const std::uint16_t Second = appReadUtf16Unit( Bytes, Offset+2, LittleEndian );
			if( !appIsLowUtf16Surrogate(Second) )
				return 0;
			Offset += 2;
			if( sizeof(TCHAR)==sizeof(UNICHAR) )
				AddedChars = 2;
		}
		else if( appIsLowUtf16Surrogate(First) )
		{
			return 0;
		}

		if( HostChars>MAXINT-1-AddedChars )
			return 0;
		HostChars += AddedChars;
	}

	Result.Empty();
	Result.Add( HostChars+1 );
	INT Out = 0;
	for( INT Offset=0; Offset<ByteCount; Offset+=2 )
	{
		const std::uint16_t First = appReadUtf16Unit( Bytes, Offset, LittleEndian );
		if( appIsHighUtf16Surrogate(First) )
		{
			...
			if( sizeof(TCHAR)==sizeof(UNICHAR) )
			{
				Result(Out++) = FromUnicode( First );
				Result(Out++) = FromUnicode( Second );
			}
			else
			{
				const std::uint32_t CodePoint
					= 0x10000u + ((static_cast<std::uint32_t>(First)-0xd800u)<<10)
					+ (static_cast<std::uint32_t>(Second)-0xdc00u);
				Result(Out++) = static_cast<TCHAR>( CodePoint );
			}
			Offset += 2;
		}
		else
...
CORE_API UBOOL appLoadFileToString( FString& Result, const TCHAR* Filename, FFileManager* FileManager )
{
	...
	const INT Size = Reader->TotalSize();
	if( Size<0 || Size>MAXINT-2 )
	{
		Reader->Close();
		delete Reader;
		return 0;
	...
	TArray<BYTE> Bytes( Size+2 );
	if( Size )
		Reader->Serialize( &Bytes(0), Size );
	UBOOL Success = !Reader->IsError();
	const UBOOL Closed = Reader->Close();
	delete Reader;
	if( !Success || !Closed )
		return 0;
	...
	Bytes(Size)   = 0;
	Bytes(Size+1) = 0;
	TArray<TCHAR>& Decoded = Result.GetCharArray();
	if( Size>=2 && Bytes(0)==0xff && Bytes(1)==0xfe )
	{
		if( !appDecodeUtf16(&Bytes(2), Size-2, 1, Decoded) )
			return 0;
	}
	else if( Size>=2 && Bytes(0)==0xfe && Bytes(1)==0xff )
	{
		if( !appDecodeUtf16(&Bytes(2), Size-2, 0, Decoded) )
			return 0;
	}
	else
	{
		...
		for( INT i=0; i<Size; i++ )
			Decoded(i) = FromAnsi( static_cast<ANSICHAR>(Bytes(i)) );
		Decoded(Size) = 0;
	}
...
CORE_API UBOOL appSaveStringToFile( const FString& String, const TCHAR* Filename, FFileManager* FileManager )
{
	...
	UBOOL SaveAsUnicode;
	TArray<BYTE> Utf16Bytes;
	if( !appEncodeUtf16LE(String, SaveAsUnicode, Utf16Bytes) )
		return 0;

	TArray<ANSICHAR> AnsiBytes;
	if( !SaveAsUnicode )
	{
		AnsiBytes.Add( String.Len() );
		for( INT i=0; i<String.Len(); i++ )
			AnsiBytes(i) = ToAnsi( (*String)[i] );
	}
	...
	if( !Ar )
		return 0;
	if( SaveAsUnicode )
		Ar->Serialize( &Utf16Bytes(0), Utf16Bytes.Num() );
	else
		Ar->Serialize( &AnsiBytes(0), AnsiBytes.Num() );

	UBOOL Success = !Ar->IsError();
	const UBOOL Closed = Ar->Close();
	delete Ar;
...
UBOOL appFindPackageFile( const TCHAR* In, const FGuid* Guid, TCHAR* Out )
{
	...
	FPushMemTag Push(TEXT("appFindPackageFile"));
...
//
// Return the leaf component of a native or legacy Unreal path.
CORE_API const TCHAR* appPathLeaf( const TCHAR* Path )
{
	if( !Path )
		return TEXT("");
	const TCHAR* Leaf = Path;
	for( const TCHAR* Cursor=Path; *Cursor; ++Cursor )
		if( *Cursor=='/' || *Cursor=='\\' )
			Leaf = Cursor + 1;
	return Leaf;
}
...
// File streaming is implemented in UnFileStream.cpp.  Keeping the complete
// lifecycle and worker implementation in one translation unit makes its
// synchronization and resource ownership explicit.
...
CORE_API const TCHAR* appSaveSlotDir()
{
	return *GSys->SaveSlotPath;
}

CORE_API const TCHAR* appSetSaveSlotDir( int iSlot )
{
	// --- Set our SaveSlotPath
	TCHAR itoa[32];
	appSprintf( itoa, TEXT("%d"), iSlot );
	GSys->SaveSlotPath = GSys->SavePath * TEXT("Slot") + FString(itoa);
	return *GSys->SaveSlotPath;
}
...
CORE_API void appInit( const TCHAR* InPackage, const TCHAR* InCmdLine, FMalloc* InMalloc, FOutputDevice* InLog, FOutputDeviceError* InError, FFeedbackContext* InWarn, FFileManager* InFileManager, FConfigCache*(*ConfigFactory)(), UBOOL RequireConfig )
{
	...
	if( !Parse( appCmdLine(), TEXT("INI="), GIni, 256 ) )
		appSprintf( GIni, TEXT("%s%s.ini"), appUserDir(), InPackage );
	bool bDefaultIniChanged = false;
	if(   RequireConfig
	   && (   GFileManager->FileSize(GIni) < 0
	       || GFileManager->GetGlobalTime(TEXT("Default.ini"))  >  GFileManager->GetGlobalTime(GIni) //ft
	      )
	  )
	{
		...
		bDefaultIniChanged = true;
	}


	// User Ini.
	TCHAR GUserIni[512]=TEXT("");
	if( !Parse( appCmdLine(), TEXT("USERINI="), GUserIni, 256 ) )
		appSprintf( GUserIni, TEXT("%sUser.ini"), appUserDir() );
	if(   RequireConfig
	   && (   GFileManager->FileSize(GUserIni) < 0
	       || GFileManager->GetGlobalTime(TEXT("DefUser.ini"))  >  GFileManager->GetGlobalTime(GUserIni) //ft
	       || bDefaultIniChanged
	      )
	  )
	{
		...
		// gk 10/15/01
	{
		TCHAR Temp[256];
		TCHAR DTemp[256];
		FConfigCache* DConfig=ConfigFactory();
		DConfig->Init(TEXT("default.ini"),GUserIni,RequireConfig);
		GConfig->GetString( TEXT("Engine.Engine"), TEXT("Language"), Temp, ARRAY_COUNT(Temp) );
		DConfig->GetString( TEXT("Engine.Engine"), TEXT("Language"), DTemp, ARRAY_COUNT(DTemp));
		GConfig->SetString( TEXT("Engine.Engine"), TEXT("Language"), DTemp );
	}
	...
	bSuccess = GConfig->GetInt( TEXT("ALAudio.ALAudioSubsystem"), TEXT("Channels"), iMaxChannels );
	GFileStream = FFileStream::Init( bSuccess ? iMaxChannels : 32 );
	...
	if( *appUserDir() )
	{
		// Subdirectory of user folder.
		GSys->SavePath = FString(appUserDir()) * TEXT("Save");
		if( GSys->Paths.Num()<=5 )
			appErrorf(TEXT("Core.System Paths are unavailable"));
		GSys->Paths(5)=GSys->SavePath * TEXT("*.usa");
	}
	else
	{
	...
	// --- Set our SaveSlotPath
	int iSlotNum;
	if( Parse( appCmdLine(), TEXT("SAVESLOT="), iSlotNum ) )
	{
		appSetSaveSlotDir( iSlotNum );
	}
	else // The SaveSlot was not set in the command line so set to Default
	{
		GSys->SaveSlotPath = GSys->SavePath;
	}
	
	if( *GSys->SaveSlotPath )
	{
		debugf( TEXT("Save Slot Path: %s"), *GSys->SaveSlotPath );
		
		// Make our SaveSlotPath
		GFileManager->MakeDirectory( *GSys->SaveSlotPath, 0 );

		// Make our SaveSlotCachePath
		GFileManager->MakeDirectory( *(GSys->SaveSlotPath * TEXT("cache")), 0 );
	}
	
	
	// --- Set our maps path	
	// Get the destination path from a Paths array entry such as ../Maps/*.unr
	FString FileExt = TEXT(".unr");
	for(int i=0; i<GSys->Paths.Num(); i++ )
	{
		if( GSys->Paths(i).Right( FileExt.Len() ) == FileExt )
		{
			INT SeparatorIndex = GSys->Paths(i).Len() - 1;
			for( ; SeparatorIndex>=0; --SeparatorIndex )
			{
				const TCHAR Ch = (*GSys->Paths(i))[SeparatorIndex];
				if( Ch=='/' || Ch=='\\' || Ch==':' )
					break;
			}
			GSys->MapPath = appBaseDir();
			GSys->MapPath = GSys->MapPath.Left( GSys->MapPath.InStr(TEXT("System")));
			GSys->MapPath += GSys->Paths(i).Left( SeparatorIndex ).Right(4);
			break;
		}
	}
	
	// Clean our our unverified persistent actor files
	LoadPersistentActorCache();

	GConfig->SetString( TEXT("Core.System"), TEXT("SavePath"), *GSys->SaveSlotPath );
	GConfig->SetString( TEXT("Core.System"), TEXT("paths"), *(GSys->SaveSlotPath * TEXT("*.usa")) );
...
CORE_API void SavePersistentActorCache( void )
{
	// *** Save our SavePersistent Actor Cache by copying all cached pa files into our imediate SaveSlotPath.
	if( !*GSys->SaveSlotPath )
		return;

	FString			Spec  = GSys->SaveSlotPath * TEXT("*_pa.*");
	TArray<FString> Files = GFileManager->FindFiles( *Spec, true, false );
	FString			PAFileName;
	
	// Delete all _pa files that are in the imediate SaveSlotPath
	for( INT j=0; Files.IsValidIndex(j); j++ )
	{
		PAFileName = GSys->SaveSlotPath * Files(j);
		if( GFileManager->FileSize( *PAFileName ) )
		{
			GFileManager->Delete( *PAFileName, false, true );
		}
	}
	Files.Empty();
	
	// Copy all the _pa files from the cache to the imediate SaveSlotPath
	Spec  = GSys->SaveSlotPath * TEXT("cache") * TEXT("*_pa.*");
	Files = GFileManager->FindFiles( *Spec, true, false );
	
	for( INT j=0; Files.IsValidIndex(j); j++ )
	{
		PAFileName = GSys->SaveSlotPath * TEXT("cache") * Files(j);
		if( GFileManager->FileSize( *PAFileName ) )
		{
			GFileManager->Copy( *(GSys->SaveSlotPath * Files(j)), *PAFileName, true, true );
		}
	}
}


CORE_API void LoadPersistentActorCache( void )
{
	// *** Load our SavePersistent Actor Cache by copying all pa files in our imediate SaveSlotPath into our cache dir
	if( !*GSys->SaveSlotPath )
		return;
	
	// Delete all cache _pa files
	FString			Spec  = GSys->SaveSlotPath * TEXT("cache") * TEXT("*.*");
	TArray<FString> Files = GFileManager->FindFiles( *Spec, true, false );
	FString			PAFileName;
	...
	// Copy pa files from our saveGameSlot to our cache
	Spec  = GSys->SaveSlotPath * TEXT("*_pa.*");
	Files = GFileManager->FindFiles( *Spec, true, false );
		
	// Copy all _pa files from our SaveGame dir to our cache dir
	for( INT j=0; Files.IsValidIndex(j); j++ )
	{
		PAFileName = GSys->SaveSlotPath * Files(j);
		if( GFileManager->FileSize( *PAFileName ) )
		{
			GFileManager->Copy( *(GSys->SaveSlotPath * TEXT("cache") * Files(j)), *PAFileName, true, true );
		}
	}
}
...
CORE_API const UNICHAR* appToUnicode( const TCHAR* Str )
{
	...
	if( !Str )
		return NULL;
	UNICHAR* UCh = appUnicodeStaticString1024();
	INT OutCount = 0;
	while( *Str && OutCount<1024-1 )
	{
		UPTRINT CodePoint = static_cast<UPTRINT>(*Str++);
		if( CodePoint<=0xd7ffu || (CodePoint>=0xe000u && CodePoint<=0xffffu) )
		{
			UCh[OutCount++] = static_cast<UNICHAR>(CodePoint);
		}
		else if( CodePoint>=0x10000u && CodePoint<=0x10ffffu )
		{
			if( OutCount+2>=1024 )
				break;
			CodePoint -= 0x10000u;
			UCh[OutCount++] = static_cast<UNICHAR>(0xd800u + (CodePoint >> 10));
			UCh[OutCount++] = static_cast<UNICHAR>(0xdc00u + (CodePoint & 0x3ffu));
		}
		else
		{
			UCh[OutCount++] = static_cast<UNICHAR>(0xfffdu);
		}
	}
	UCh[OutCount] = 0;
	return UCh;
...
CORE_API const TCHAR* appFromAnsi( TArray<TCHAR> & Ch, const ANSICHAR* ACh )
{
#if defined(__APPLE__) || UNICODE
	guard(appFromAnsi);
	if( !ACh )
...
CORE_API const TCHAR* appFromUnicode( const UNICHAR* UCh )
{
	...
	INT InCount = 0;
	INT OutCount = 0;
	while( UCh[InCount] && OutCount<1024-1 )
	{
		const UNICHAR First = UCh[InCount++];
		UPTRINT CodePoint;
		if( First>=0xd800u && First<=0xdbffu )
		{
			const UNICHAR Second = UCh[InCount];
			if( Second>=0xdc00u && Second<=0xdfffu )
			{
				InCount++;
				CodePoint = 0x10000u
					+ ((static_cast<UPTRINT>(First)-0xd800u) << 10)
					+ (static_cast<UPTRINT>(Second)-0xdc00u);
			}
			else
			{
				CodePoint = 0xfffdu;
		...
		Ch[OutCount++] = static_cast<TCHAR>(CodePoint);
	}
	Ch[OutCount] = 0;
	return Ch;
	unguard;
