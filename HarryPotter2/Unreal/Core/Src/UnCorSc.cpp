...
const TCHAR* FFrame::Describe() const
{
	TCHAR* Buffer = appStaticString1024();
	*Buffer = 0;
	if( Object )
	{
		// Print each function in stack, omitting outer package.
		for( const FFrame* Frame = this; Frame; Frame = Frame->Parent )
		{
			if( Frame->Node )
			{
				if( Frame != this )
					appStrcat( Buffer, TEXT("<") );
				appSprintf( Buffer+appStrlen(Buffer), TEXT("%s(%d)"),
					Frame->Node->GetPathName( Object->GetClass()->GetOuter() ),
					Frame->Node->Line );
			}
		}
	}
	...
	if( StateFrame )
	{
		...
		if( FindLabel != NAME_None )
		{
			for( UState* SourceState=StateFrame->StateNode; SourceState; SourceState=SourceState->GetSuperState() )
			{
				...
				{
					INT LabelOffset = SourceState->LabelTableOffset;
					for( ; ; )
					{
						INT LabelNameIndex;
						INT LabelCodeOffset;
						appMemcpy( &LabelNameIndex, &SourceState->Script(LabelOffset), appCheckedIntSize(sizeof(LabelNameIndex)) );
						appMemcpy( &LabelCodeOffset, &SourceState->Script(LabelOffset+appCheckedIntSize(sizeof(LabelNameIndex))), appCheckedIntSize(sizeof(LabelCodeOffset)) );
						if( LabelNameIndex==NAME_None )
							break;
						if( FName(static_cast<EName>(LabelNameIndex))==FindLabel )
						{
							StateFrame->Node = SourceState;
							StateFrame->Code = &SourceState->Script(LabelCodeOffset);
							return 1;
						}
						LabelOffset += appCheckedIntSize(sizeof(LabelNameIndex)) + appCheckedIntSize(sizeof(LabelCodeOffset));
					}
				}
			}
		}
...
void UObject::execDynArrayElement( FFrame& Stack, RESULT_DECL )
{
	...
	if( ArrayProp && GPropAddr )
	{
		...
		{
			// Display out-of-bounds warning and continue on with index clamped to valid range.
			Stack.Logf( TEXT("Accessed array out of bounds (%i/%i)"), Index, Array->Num() );
			Index = 0;
		}
		else if( Index>=Array->Num() )
		{
			Array->AddZeroed( ArrayProp->ElementSize, Index+1 - Array->Num() );
		}

		// Update address.
		GPropAddr = Array->GetData() + Index * ArrayProp->ElementSize;
		guard(ArrayElementCopySingleValue);
		if( Result )
		{
...
void UObject::execDynArrayCount( FFrame& Stack, RESULT_DECL )
{
	guardSlow(UObject::execDynArrayCount);
...
IMPLEMENT_FUNCTION( UObject, EX_DynArrayCount, execDynArrayCount );
...
void UObject::execSwitch( FFrame& Stack, RESULT_DECL )
{
	...
	appMemzero( SwitchBuffer, appCheckedIntSize(sizeof(FString)) );
...
void UObject::execLet( FFrame& Stack, RESULT_DECL )
{
	...
	if( !GPropAddr )
	{
		...
		GPropAddr = Crud;
		appMemzero( GPropAddr, appCheckedIntSize(sizeof(FString)) );
	}
...
void UObject::execContext( FFrame& Stack, RESULT_DECL )
{
	...
	{
		Stack.Logf( TEXT("Accessed None: %s"), GProperty ? GProperty->GetName() : TEXT("Unknown") );
		INT wSkip = Stack.ReadWord();
		BYTE bSize = *Stack.Code++;
		Stack.Code += wSkip;
		GPropAddr = NULL;
...
void UObject::execNameToString( FFrame& Stack, RESULT_DECL )
{
	...
	// hp1-parity: oversized parm buffer absorbs pointer-width
	// copies from era-different property types.
	BYTE N_Buf[8];
	appMemzero( N_Buf, sizeof(N_Buf) );
	Stack.Step( Stack.Object, N_Buf );
	FName N;
	appMemcpy( &N, N_Buf, sizeof(FName) );
	*(FString*)Result = *N;
	unguardexecSlow;
}
...
void UObject::execStringToName( FFrame& Stack, RESULT_DECL )
{
	...
	P_GET_STR(Str);
	*(FName*)Result = FName( *Str, FNAME_Find );
	unguardexecSlow;
}
IMPLEMENT_FUNCTION( UObject, EX_StringToName, execStringToName );
...
IMPLEMENT_FUNCTION( UObject, 1030, execOrthoRotation );
#else
...
#if 1 //Fix added by Legend on 4/12/2000
IMPLEMENT_FUNCTION( UObject, 1031, execNormalize );
#else
...
IMPLEMENT_FUNCTION( UObject, INDEX_NONE, execGetLanguage);
...
void UObject::execDynamicLoadObject( FFrame& Stack, RESULT_DECL )
{
	...
	FPushMemTag Push(TEXT("DynamicLoadObject"));
