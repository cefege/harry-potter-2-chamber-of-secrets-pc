...
,	Name        ( NAME_None					)
{
	...
	check(sizeof(_Linker)>=sizeof(InName       ));
	*(const TCHAR**)&Outer   = InPackageName;
	*(const TCHAR**)&_Linker = InName;
	HashNext                       = GAutoRegister;
	GAutoRegister                  = this;
...
public:
	...
	void SerializeCapsString( const TCHAR* Str )
	{
		while( *Str )
		{
			const TCHAR HostChars[2] = { appToUpper(*Str++), 0 };
			const UNICHAR* Wire = appToUnicode(HostChars);
			while( Wire && *Wire )
			{
				UNICHAR Unit = *Wire++;
				*this << Unit;
	...
	if( !Ar.IsTrans() )
	{
		if( GetFlags() & RF_HasStack )
		{
			...
			if( StateFrame->Node )
			{
				...
				if( StateFrame->Code )
				{
					const PTRINT CodeOffset = StateFrame->Code - &StateFrame->Node->Script(0);
					check( CodeOffset>=0 );
					Offset = appCheckedIntSize( static_cast<SIZE_T>(CodeOffset) );
				}
				Ar << AR_INDEX(Offset);
...
void UObject::InitProperties( BYTE* Data, INT DataCount, UClass* DefaultsClass, BYTE* Defaults, INT DefaultsCount )
{
	...
	const INT UObjectSize = appCheckedIntSize(sizeof(UObject));
	check(DataCount>=UObjectSize);
	INT Inited = UObjectSize;
	...
	if( DefaultsClass )
	{
		...
		{
			guard(InitProperty);
			if( P->Offset < UObjectSize )
				appErrorf( TEXT("Constructor-linked property %s has invalid host offset %i"), P->GetFullName(), P->Offset );
			...
			unguardf(( TEXT("%s offset=%i size=%i"), P->GetFullName(), P->Offset, P->GetSize() ));
		}
	}
	unguardSlow;
...
void UObject::ExitProperties( BYTE* Data, UClass* Class )
{
	...
	for( P=Class->ConstructorLink; P; P=P->ConstructorLinkNext )
		if( P->Offset )			// Partial fix for load/save/import problem in incremental compile.
...
void UObject::InitClassDefaultObject( UClass* InClass )
{
	...
	appMemset( this, 0, appCheckedIntSize(sizeof(UObject)) );
...
void UObject::Register()
{
	...
	const TCHAR* InName  = *(const TCHAR**)&_Linker;
...
void UObject::StaticInit()
{
	...
	// Development.
	FPushMemTag Push(TEXT("UObject::StaticInit"));
...
void UObject::ProcessRegistrants()
{
	...
	if( GObjRegisterCount>=0 )
	{
		...
		GObjRegisterCount = -1;
		FPushMemTag Push(TEXT("ProcessRegistrants"));
		for( ; GAutoRegister; GAutoRegister=GAutoRegister->HashNext )
...
void UObject::StaticExit()
{
	...
	if( ParseParam(appCmdLine(),TEXT("PROFILE")) )
	{
		...
		appQsort( &List(0), List.Num(), appCheckedIntSize(sizeof(List(0))), (QSORT_COMPARE)CompareFunctions );
		...
		for( i=0; i<List.Num(); i++ )
		{
   			debugf
   			(
   				TEXT("%80s  %12.3f  %12.3f  %12.3f  %12.0f  %6.3f"),
   
   				List(i)->GetPathName(),
   				1000.0*1000.0*GSecondsPerCycle*(FLOAT)List(i)->Cycles/(FLOAT)GTicks,
   				(FLOAT)List(i)->Cycles/(FLOAT)List(i)->Calls,
   				(FLOAT)List(i)->Calls/(FLOAT)GTicks,
   				(FLOAT)List(i)->Calls,
				(FLOAT)List(i)->Cycles / TotalCycles * 100
   			);
		}
#else
...
#if 1 //Logging -- sort objects by name -- added by Legend on 4/12/2000
CORE_API QSORT_RETURN CDECL CompareObjectNames( const UObject** A, const UObject** B )
{
	QSORT_RETURN RetVal;

	// pull out base classes
	UClass* BaseClassA = (*A)->GetClass();
	UClass* BaseClassB = (*B)->GetClass();

	RetVal = appStrcmp( BaseClassA->GetName(), BaseClassB->GetName() );	

	if( RetVal == 0 )
	{
		// same base classes -- pull out object nums (so xxx9 < xxx10 etc.)
		INT Val1 = appAtoi( &(*A)->GetName()[ appStrlen(BaseClassA->GetName()) ] );
		INT Val2 = appAtoi( &(*B)->GetName()[ appStrlen(BaseClassB->GetName()) ] );
...
{
#if 1 //Logging -- if Max's tie, use name to break tie for easier object finding -- added by Legend on 4/12/2000
	QSORT_RETURN RetVal = B->Max>A->Max ? 1 : B->Max<A->Max ? -1 : 0;
	...
	return RetVal;
#else
	return B->Max>A->Max ? 1 : B->Max<A->Max ? -1 : 0;
#endif
}
...
UBOOL UObject::StaticExec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("OBJ")) )
	{
		...
		else if( ParseCommand(&Str,TEXT("LIST")) )
		{
			...
			if( Objects.Num() )
			{
				appQsort( &Objects(0), Objects.Num(), appCheckedIntSize(sizeof(Objects(0))), (QSORT_COMPARE)CompareSubItems );
#if 1 //Logging added by Legend on 4/12/2000
				Ar.Log( TEXT(" ") );
				Ar.Logf( TEXT("%-78s %10s %10s"), TEXT("Object"), TEXT("NumBytes"), TEXT("MaxBytes") );

				UBOOL bShowDeleted = ParseCommand(&Str,TEXT("VERBOSE"));
				for( INT i=0; i<Objects.Num(); i++ )
				{
					if( bShowDeleted || !Objects(i).Object->IsPendingKill() )
					{
						Ar.Logf( TEXT("%-78s %10zu %10zu %s"), Objects(i).Object->GetFullName(), Objects(i).Num, Objects(i).Max, Objects(i).Object->IsPendingKill() ? TEXT("X") : TEXT(" ") );
					}
				...
				for( INT i=0; i<Objects.Num(); i++ )
					Ar.Logf( TEXT("%60s % 10zu % 10zu"), Objects(i).Object->GetFullName(), Objects(i).Num, Objects(i).Max );
				Ar.Log( TEXT("") );
#endif
			}
			if( List.Num() )
			{
				appQsort( &List(0), List.Num(), appCheckedIntSize(sizeof(List(0))), (QSORT_COMPARE)CompareItems );
#if 1 //Logging added by Legend on 4/12/2000
				Ar.Logf( TEXT("%-30s %6s %10s %10s"), TEXT("Class"), TEXT("Count"), TEXT("NumKBytes"), TEXT("MaxKBytes") );
				for( INT i=0; i<List.Num(); i++ )
				{
					Ar.Logf(TEXT("%-30s %6i %10zu %10zu"), List(i).Class->GetName(), List(i).Count, List(i).Num/1024, List(i).Max/1024 );
				}
				Ar.Log( TEXT(" ") );
#else
				Ar.Logf(TEXT(" %30s % 6s % 10s  % 10s "), TEXT("Class"), TEXT("Count"), TEXT("NumBytes"), TEXT("MaxBytes") );
				for( INT i=0; i<List.Num(); i++ )
					Ar.Logf(TEXT(" %30s % 6i % 10zuK % 10zuK"), List(i).Class->GetName(), List(i).Count, List(i).Num/1024, List(i).Max/1024 );
				Ar.Log( TEXT("") );
#endif
			}
#if 1 //Logging added by Legend on 4/12/2000
			Ar.Logf( TEXT("%i Objects (%.3fM / %.3fM)"), Total.Count, (FLOAT)Total.Num/1024.0/1024.0, (FLOAT)Total.Max/1024.0/1024.0 );
			Ar.Logf( TEXT("%i Deleted Objects" ), DeletedCount );
#else
			Ar.Logf( TEXT("%i Objects (%.3fM / %.3fM)"), Total.Count, (FLOAT)Total.Num/1024.0/1024.0, (FLOAT)Total.Max/1024.0/1024.0 );
#endif
			return 1;
...
#if 1 //Fix added by Legend on 4/12/2000
	//
	// Pass "STRICT" on the command line to verify that no warnings are being ignored
	//
	if( GIsStrict )
	{
		LoadFlags = LOAD_NoFail;
	}
#endif
...
)
{
	...
	{
		debugf( LocalizeError("Sandbox"), InOuter->GetName() );
		Result=NULL;
	}
...
//
// Object sorting during load.
//
static UObject* CurObj;
inline INT Compare( UObject* A, UObject* B )
{
	// Sort remaining objects in order to minimise seeks, using last-read object as base.
	checkSlow( CurObj != NULL );
	if( A->GetLinker() != B->GetLinker() )
	{
		// Different linkers.
		if( A->GetLinker() == CurObj->GetLinker() )
			return -1;		// A < B.
		if( B->GetLinker() == CurObj->GetLinker() )
			return 1;		// B < A.
		return A->GetLinker() > B->GetLinker() ? 1 : -1;
	}
	INT AOffset = A->GetLinker()->ExportMap( A->GetLinkerIndex() ).SerialOffset,
		BOffset = B->GetLinker()->ExportMap( B->GetLinkerIndex() ).SerialOffset;

	if( A->GetLinker() == CurObj->GetLinker() )
	{
		// Both same linker as cur. Order items after Cur first.
		INT CurOffset = CurObj->GetLinker()->ExportMap( CurObj->GetLinkerIndex() ).SerialOffset;
		if( AOffset > CurOffset && BOffset < CurOffset )
			return -1;		// A < B.
		if( AOffset < CurOffset && BOffset > CurOffset )
			return 1;		// B < A.
	}
...
void UObject::EndLoad()
{
	...
	if( --GObjBeginLoadCount == 0 )
	{
		try
		{
			...
			{
				if( CurObj && GObjLoaded.Num() > LastSorted && i < GObjLoaded.Num()-1 )
				{
					// Sort new objects by archive/address.
					Sort( &GObjLoaded(i), GObjLoaded.Num() - i );
					LastSorted = GObjLoaded.Num();
				}
			...
			for( INT i=0; i<GObjLoaded.Num(); i++ )
			{
				FPushMemTag Tag( GObjLoaded(i)->GetClass()->GetName() );
...
public:
	...
	FArchive& operator<<( UObject*& Obj )
	{
		...
		{
			debugf( NAME_Warning, TEXT("trying to archive deleted object: %s"), Obj->GetName() );
		}
...
UBOOL UObject::SavePackage( UObject* InOuter, UObject* Base, DWORD TopLevelFlags, const TCHAR* Filename, FOutputDevice* Error, ULinkerLoad* Conform )
{
	...
	try
	{
		...
		*Linker << Linker->Summary;
		...
		appQsort( &Linker->NameMap(FirstSort), Linker->NameMap.Num()-FirstSort, appCheckedIntSize(sizeof(Linker->NameMap(0))), LinkerNameSort );
		unguard;
		...
		guard(SortImports);
		GTempSave = Linker;
		appQsort( &Linker->ImportMap(0), Linker->ImportMap.Num(), appCheckedIntSize(sizeof(Linker->ImportMap(0))), LinkerImportSort );
		unguard;
		...
		appQsort( &Linker->ExportMap(FirstSort), Linker->ExportMap.Num()-FirstSort, appCheckedIntSize(sizeof(Linker->ExportMap(0))), LinkerExportSort );
...
)
{
	...
	const TCHAR* ClassName = GObjRegisterCount > 0
		? InClass->GetName()
		: *InClass->GetFName();
	FPushMemTag Push(ClassName);
	...
	UBoolProperty* BoolProperty          = Cast<UBoolProperty>( Obj );
	INT      PropertiesSize            = 0;
	INT      PropertiesAlign           = 0;
	INT      PropertyOffset            = 0;
	UBOOL    IsCppProperty              = 0;
	BITFIELD PropertyBitMask             = 0;
	...
	{
		// Create a new object.
		Obj = Ptr ? Ptr : (UObject*)appMalloc( InClass->GetPropertiesSize(), ClassName );
	}
	else
	{
		...
		if( Cls )
		{
			...
			PropertiesSize   = Cls->PropertiesSize;
			PropertiesAlign  = Cls->PropertiesAlign;
		}
		if( Property )
		{
			PropertyOffset = Property->Offset;
			IsCppProperty  = Property->CppProperty;
			if( BoolProperty )
				PropertyBitMask = BoolProperty->BitMask;
		}
		// Destroy the object.
		Obj->~UObject();
	...
	if( Cls )
	{
		...
		Cls->PropertiesSize    = PropertiesSize;
		Cls->PropertiesAlign   = PropertiesAlign;
	}
	if( Property )
	{
		Property->Offset      = PropertyOffset;
		Property->CppProperty = IsCppProperty;
		if( BoolProperty )
			BoolProperty->BitMask = PropertyBitMask;
	}
...
)
{
	...
	// Allocate the object.
	FPushMemTag Push(InClass->GetName());
...
{
public:
	FArchiveTagUsed( UObject* InRefObj = NULL )
	: Context( NULL ), RefObj( InRefObj )
	{
	...
	inline UObject* GetReferencing() const
	{
		return RefObj;
	}
private:
	FArchive& operator<<( UObject*& Object )
	{
		...
		UObject* Obj;
		appMemcpy(&Obj, &Object, appCheckedIntSize(sizeof(Obj)));
		...
		appMemcpy(&Object, &Obj, appCheckedIntSize(sizeof(Object)));
		return *this;
		unguardSlow;
	}
	FArchive& operator<<( FName& Name )
	{
		...
		FName LocalName;
		appMemcpy(&LocalName, &Name, appCheckedIntSize(sizeof(LocalName)));
		LocalName.ClearFlags( RF_Unreachable );
		appMemcpy(&Name, &LocalName, appCheckedIntSize(sizeof(LocalName)));
		#else
...
void UObject::PurgeGarbage()
{
	...
	for( INT i=GObjObjects.Num()-1; i>=0; i-- )
	{
...
UBOOL UObject::IsReferenced( UObject*& Obj, DWORD KeepFlags, UBOOL IgnoreReference )
{
	...
	// Tag all garbage.
	FArchiveTagUsed TagUsedAr(OriginalObj);
...
FArchive& operator<<( FArchive& Ar, FCompactIndex& I )
{
	...
	else
	{
		...
		DWORD V = Original < 0 ? 0u - static_cast<DWORD>(Original) : static_cast<DWORD>(Original);
...
void UObject::ImportPostProcess( const TCHAR* Filename, const TCHAR* Parms )
{
}
