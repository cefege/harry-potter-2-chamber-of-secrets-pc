...
)
{
	...
	checkSlow( (reinterpret_cast<UPTRINT>(Result) & static_cast<UPTRINT>(Alignment-1)) == 0 );
	...
	if( ParseCommand(&Cmd,TEXT("DUMPCACHE")) )
	{
		for( FCacheItem* Item=CacheItems; Item!=LastItem; Item=Item->LinearNext )
		{
			...
			if( Item->Cost >= COST_INFINITE )
				...
				Descr=TEXT("Fresh");
			Ar.Logf( TEXT("%02X [%i]: %s"), (BYTE)Item->Id, Item->GetSize(), Descr );
		}
		return 1;
	}
	else return 0;
